...
const transportAliases = {
  ...
  VisionOptions: "WorkmarketOptionsDTO"
}
...
const pathSegments = (filePath) => path.relative(backendRoot, filePath).split(path.sep)
...
const getSourceDomain = (filePath) => toPascalCase(pathSegments(filePath)[0] ?? "Common")
...
async function parseEnumFile(filePath) {
  ...
  return {enumName, values, filePath, sourceDomain: getSourceDomain(filePath)}
}

function buildEnumLookup(enums) {
  const byName = new Map()

  for (const entry of enums) {
    if (!byName.has(entry.enumName)) {
      byName.set(entry.enumName, [])
    }

    byName.get(entry.enumName).push(entry)
  }

  // Keep one canonical name for shared frontend imports, but namespace colliding enums by backend domain.
  ...
  for (const [enumName, entries] of byName.entries()) {
    if (entries.length === 1) {
      typeNames.set(entries[0], enumName)
      continue
    }

    entries.forEach((entry) => {
      typeNames.set(entry, `${entry.sourceDomain}${enumName}`)
    })
  }
...
function buildOutput(enums, enumLookup) {
  ...
  for (const entry of enums) {
    const {enumName, values, sourceDomain} = entry
    const entries = enumLookup.byName.get(enumName) ?? []
    const exportedName = enumLookup.typeNames.get(entry) ?? enumName
    const constantName = entries.length > 1 ? `${toUpperSnakeCase(sourceDomain)}_${toUpperSnakeCase(enumName)}_VALUES` : `${toUpperSnakeCase(enumName)}_VALUES`
    const valuesLiteral = values.map((value) => `"${value}"`).join(", ")
    lines.push(`export const ${constantName} = [${valuesLiteral}] as const`)
    lines.push(`export type ${exportedName} = typeof ${constantName}[number]`)
    lines.push("")
  }

  for (const [enumName, entries] of enumLookup.byName.entries()) {
    if (entries.length < 2) {
      continue
    }

    const preferred = entries[0]
    const preferredTypeName = enumLookup.typeNames.get(preferred)
    const preferredConstantName = `${toUpperSnakeCase(preferred.sourceDomain)}_${toUpperSnakeCase(enumName)}_VALUES`
    const aliasConstantName = `${toUpperSnakeCase(enumName)}_VALUES`
    const aliasTypeName = enumName

    lines.push(`export const ${aliasConstantName} = ${preferredConstantName}`)
    lines.push(`export type ${aliasTypeName} = ${preferredTypeName}`)
    lines.push("")
  }
...
function mapJavaTypeToTs(rawType, enumLookup, dtoLookup, sourceDomain) {
  ...
  if (SIMPLE_TYPE_MAP.has(simpleType)) {
    return SIMPLE_TYPE_MAP.get(simpleType)
  }

  const enumEntries = enumLookup.byName.get(simpleType)
...
async function parseDtoFile(filePath) {
  ...
  const sourceDomain = getSourceDomain(filePath)
...
async function main() {
  ...
  const enumLookup = buildEnumLookup(parsedEnums)
