...
const checks = [
  ...
  ["people/circle privacy explanation", read("src/modules/app-shell/views/CirclesView.vue").includes("Circles control shared visibility") && read("src/modules/app-shell/views/CirclesView.vue").includes("Exact location")],
  ...
  ["shell read surfaces fail closed", !read("src/modules/app-shell/shellSurfaceData.ts").includes("safeRequest") && read("src/modules/app-shell/shellSurfaceData.ts").includes("backendRequest")],
  ...
  , ["shell core navigation model", read("src/modules/app-shell/shellRouteRegistry.ts").includes('primaryNavigationSurfaceIds: AppSurfaceId[] = ["home", "work", "business", "business-discovery", "things", "circles", "rides"]')]
  , ["shell secondary navigation model", read("src/modules/app-shell/shellRouteRegistry.ts").includes('secondaryNavigationSurfaceIds: AppSurfaceId[] = ["chat", "calendar", "profile"]')]
  , ["shell exposes account navigation", read("src/modules/app-shell/views/AuthenticatedShellView.vue").includes("<AccountMenu") && read("src/modules/app-shell/components/AccountMenu.vue").includes('to="/profile/settings"')]
  ...
  , ["shared action confirmation primitive", read("src/modules/app-shell/components/AppActionDialog.vue").includes("role=\"dialog\"") && read("src/modules/app-shell/components/AppActionDialog.vue").includes("@keydown.escape") && read("src/modules/app-shell/components/AppIconButton.vue").includes("aria-label")]
  ...
  , ["chat mobile back affordance", read("src/modules/app-shell/views/ChatSurfaceView.vue").includes(">Back</") && read("src/modules/app-shell/views/ChatSurfaceView.vue").includes("chat-surface__back")]
  ...
  , ["reduced motion and visible focus", read("src/styles/base.css").includes("prefers-reduced-motion") && read("src/styles/base.css").includes(":focus-visible") && read("src/styles/base.css").includes("outline: var(--focus-ring)") && read("src/styles/base.css").includes("forced-colors: active")]
  ...
  , ["home cards have backend actions", read("src/modules/app-shell/shellSurfaceData.ts").includes("actions?: WorkspaceCommandItem[]") && read("src/modules/app-shell/shellSurfaceData.ts").includes("getWorkspaceCommandCatalog") && read("src/modules/app-shell/views/HomeHubView.vue").includes("section.actions")]
  ...
  , ["rides commute exposes backend weekdays", read("src/modules/app-shell/views/RidesView.vue").includes("commute.weekdays") && read("src/modules/rides/api/ridesApi.ts").includes("weekdays: number[]")]
  , ["things create preserves full request DTO", read("src/modules/app-shell/views/ThingsDiscoveryView.vue").includes("createThingListing: thingsApi.createListing")]
  , ["multiline backend DTO fields reach generated contracts", read("scripts/generate-vision-contracts.mjs").includes("([^;]+?)") && read("src/contracts/generated/themuffinmanContract.ts").includes("images?: string[]")]
