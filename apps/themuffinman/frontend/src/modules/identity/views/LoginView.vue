...
<template>
  <main class="auth-terminal identity-surface">
    <section class="auth-terminal__panel identity-surface__panel">
      ...
      <p class="auth-terminal__copy identity-surface__copy">Enter your account details to open your workspace.</p>

      <form class="auth-terminal__form identity-surface__form" :aria-busy="isSubmitting || undefined" @submit.prevent="login"><AppFormField label="Email" required><input v-model="email" aria-label="Email" class="auth-terminal__input identity-surface__input" type="email" autocomplete="email" :disabled="isSubmitting" required /></AppFormField><AppFormField label="Password" required><input v-model="password" aria-label="Password" class="auth-terminal__input identity-surface__input" type="password" autocomplete="current-password" :disabled="isSubmitting" required /></AppFormField><AppStatus v-if="isSubmitting" message="Opening your workspace…" busy /><AppStatus v-else-if="error" :message="error" tone="error" retry @retry="login" /><AppFormFooter><template #primary><button type="submit" :disabled="isSubmitting">{{ isSubmitting ? "Opening…" : "Enter" }}</button></template></AppFormFooter></form>

      <p class="auth-terminal__linkline identity-surface__linkline">
        Need an account?
        <RouterLink to="/register">Register</RouterLink>
      </p>
      <p class="auth-terminal__linkline identity-surface__linkline"><RouterLink to="/recover">Forgot your password?</RouterLink></p>
    </section>
  </main>
</template>
...
.auth-terminal__panel {
  width:min(100%,31rem);display:grid;gap:var(--space-3);padding:clamp(var(--space-4),4vw,var(--space-6));border:1px solid var(--border-subtle);border-radius:var(--radius-surface);background:var(--surface-base);box-shadow:var(--shadow-overlay)
}
