...
export const useCirclesMutationActions = (
  ...
  const createCircle = async () => {
    await runCircleMutation(
      () => visionApi.createCircle({name: normalizeSearchQuery(state.newCircleName.value)}),
  ...
  const deleteCircle = async (circleId: number) => {
    await runCircleMutation(
      () => visionApi.deleteCircle(circleId),
  ...
  const renameCircle = async (circleId: number, name: string) => {
    await runCircleMutation(
      () => visionApi.updateCircle(circleId, {name: normalizeSearchQuery(name)}),
  ...
  const saveConnectionCircles = async (connection: CircleContact) => {
    ...
    await runCircleMutation(
      () => visionApi.updateConnectionCircles(connection.userId, {circleIds: nextCircleIds}),
  ...
  const bulkUpdateConnections = async (circleId: number, userIds: number[], action: "ADD" | "REMOVE") => {
    await runCircleMutation(
      () => visionApi.updateConnectionCirclesBulk({circleId, userIds, action}),
  ...
  const sendRequest = async (id: number) => {
    await runCircleMutation(() => visionApi.createCircleRequest({recipientId: id}), "Could not send invite.")
  }

  const blockUser = async (id: number) => {
    await runCircleMutation(() => visionApi.blockCircleUser({blockedUserId: id}), "Could not block user.")
  }

  const unblockUser = async (id: number) => {
    await runCircleMutation(() => visionApi.unblockCircleUser(id), "Could not unblock user.")
  }

  const acceptRequest = async (requestId: number) => {
    await runCircleMutation(() => visionApi.acceptCircleRequest(requestId), "Could not accept invite.")
  }

  const removeRequest = async (requestId: number, tone: "success" | "warning" = "warning") => {
    await runCircleMutation(
      () => visionApi.deleteCircleRequest(requestId),
