...
export const useCirclesDataLoader = (state: CirclesDataLoaderState) => {
  const loadOverview = async () => {
    try {
      const overview = await visionApi.getCircleOverview()
  ...
  const loadCircles = async () => {
    try {
      state.circles.value = await visionApi.getCircleGroups()
    } catch (requestError) {
      state.error.value = getApiErrorMessage(requestError, "Could not load circles.")
    }
  }
  ...
  const loadConnectionsPage = async () => {
    ...
    try {
      state.connectionsPageData.value = await visionApi.getCircleConnectionsPage({
        q: query,
        circleId,
        unassigned,
  ...
  const loadInboxPage = async () => {
    ...
    try {
      const [incomingResponse, outgoingResponse] = await Promise.all([
        visionApi.getIncomingCircleRequestsPage({
          q: query,
          page: state.incomingPage.value - 1,
          size: state.pageSize
        }),
        visionApi.getOutgoingCircleRequestsPage({
          q: query,
  ...
  const loadBlockedPage = async () => {
    ...
    try {
      state.blockedPageData.value = await visionApi.getBlockedCircleUsersPage({
        q: query,
  ...
  const loadSuggestions = async () => {
    ...
    try {
      if (state.discoverHasQuery.value) {
        state.searchResults.value = (await visionApi.searchCircleUsersPage({
      ...
      state.inviteCandidates.value = (await visionApi.getInviteCandidatesPage({
        page: 0,
        size: 12
      })).items
  ...
  const loadNearbyPage = async () => {
    try {
      state.nearbyPageData.value = await visionApi.getNearbyCircleUsersPage({
