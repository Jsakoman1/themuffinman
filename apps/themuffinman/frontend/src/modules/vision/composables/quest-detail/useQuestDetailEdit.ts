...
import {visionApi, type CircleGroup, type Quest, type QuestAudienceOption, type QuestLocationVisibilityOption} from "../../api/visionApi.ts"
...
export const useQuestDetailEdit = (state: QuestDetailEditState) => {
  ...
  const saveEdits = async () => {
    ...
    try {
      await visionApi.updateQuest(state.quest.value.id, {
  ...
  const loadEditMetadata = async () => {
    try {
      const [options, circles] = await Promise.all([
        visionApi.getAppUserOptions(),
        visionApi.getCircleGroups()
      ])
