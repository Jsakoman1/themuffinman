...
const props = withDefaults(defineProps<{title: string; subtitle?: string; open?: boolean; detailLabel?: string; loading?: boolean; actionOutcome?: string}>(), {open: false, detailLabel: "Open full detail", loading: false, actionOutcome: ""})
...
<template>
    ...
    <div class="object-preview__body" aria-live="polite" :aria-busy="loading || undefined"><p v-if="loading" class="object-preview__loading">Loading preview…</p><template v-else><slot /><p v-if="actionOutcome" class="object-preview__outcome"><strong>What happens next:</strong> {{ actionOutcome }}</p></template></div>
    <footer>
