...
// Empty states distinguish absence, visibility, filtering, failure, conflict, and permission.
...
<template><section class="app-empty-state" data-empty-state="true" :data-empty-reason="reason" aria-live="polite"><p class="app-empty-state__reason">{{ reasonLabel }}</p><h2>{{ title }}</h2><p v-if="message">{{ message }}</p><div v-if="$slots.actions" class="app-empty-state__actions"><slot name="actions" /></div></section></template>
