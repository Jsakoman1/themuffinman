...
<template data-command-model="backend-prepared-shortcut-aware">
  <details :open="openPanel" class="global-search-entry" data-command-surface="backend-prepared-cmd-k" @toggle="openPanel = ($event.currentTarget as HTMLDetailsElement).open">
    <summary ref="summaryButton" aria-label="Search and commands" :aria-expanded="openPanel" aria-controls="global-command-panel" title="Search and commands (Ctrl or Command K)">⌕ <span>Search</span></summary>
      <div id="global-command-panel" class="global-search-entry__panel" role="dialog" aria-modal="false" aria-label="Command center">
      <p class="global-search-entry__scope">Search your workspace</p>
      <form class="global-search-entry__form" @submit.prevent="search()"><input v-model="query" type="search" placeholder="Work, people, business, things…" aria-label="Search across modules"><select v-model="selectedFamily" aria-label="Filter search family"><option :value="undefined">All modules</option><option v-for="family in (result?.availableEntityFamilies || [])" :key="family" :value="family">{{ family }}</option></select><button class="global-search-entry__submit" type="submit" :disabled="loading">{{ loading ? "Searching" : "Search" }}</button></form>
      <details class="shortcut-section">
        <summary>Shortcuts</summary>
        <p v-if="catalogLoading" class="search-status" role="status" aria-live="polite">Loading permitted shortcuts…</p>
        ...
        <p v-else class="search-status" role="status">No shortcuts are available for this account.</p>
      </details>
      <div v-if="error" class="error-state" role="alert"><p class="error">{{ error }}</p><button class="search-secondary-action" type="button" @click="error = ''; focusSearch()">Retry</button></div>
      <div v-else-if="result" class="summary-row"><p class="summary">{{ result.summary }}</p><button class="search-secondary-action" type="button" @click="saveIntent">{{ saved ? "Saved" : "Save search" }}</button></div>
      <div v-if="result?.items?.length" class="results"><div v-for="item in result.items" :key="`${item.entityFamily}-${item.targetId}`" class="result-row"><input type="checkbox" :checked="selectedKeys.includes(selectionKey(item))" :aria-label="`Select ${item.title} for comparison`" @change="toggleSelection(item)"><button class="result-row__open" type="button" @click="open(item)"><span>{{ item.entityFamily }}</span><strong>{{ item.title }}</strong><small>{{ item.matchSummary || item.summary }}</small></button></div><button type="button" class="compare-action" :disabled="selectedKeys.length === 0 || loading" @click="compare">Compare selected ({{ selectedKeys.length }}/3)</button></div>
      ...
      <button v-if="result?.hasMore && !loading" type="button" class="search-secondary-action next-page" @click="search(searchPage + 1)">Show more results</button>
      ...
      <RouterLink class="saved-link" to="/search/saved">Saved searches</RouterLink>
    </div>
  </details>
</template>

<style scoped>
.global-search-entry{position:relative}.global-search-entry>summary{cursor:pointer;list-style:none;padding:var(--space-1) var(--space-2);border:1px solid var(--control-border);border-radius:var(--radius-control);background:transparent;color:var(--text-muted);font-size:var(--text-size-body);font-weight:var(--text-weight-semibold)}.global-search-entry>summary:hover{border-color:var(--control-border-active);color:var(--text)}.global-search-entry>summary::-webkit-details-marker{display:none}.global-search-entry__panel{position:fixed;inset-inline-start:max(var(--space-3),env(safe-area-inset-left));inset-inline-end:max(var(--space-3),env(safe-area-inset-right));top:calc(var(--workspace-header-height) + var(--space-2));z-index:var(--z-popover);width:min(34rem,calc(100vw - (2 * var(--space-3))));max-height:calc(100dvh - var(--workspace-header-height) - (2 * var(--space-3)));overflow:auto;display:grid;gap:var(--space-3);padding:var(--space-3);border:1px solid var(--border-strong);border-radius:var(--radius-surface);background:var(--surface-raised);box-shadow:var(--shadow-overlay);box-sizing:border-box}.global-search-entry__scope,.commands section>p{margin:0;color:var(--text-soft);font-size:var(--text-size-label);font-weight:var(--text-weight-semibold);letter-spacing:var(--tracking-label);text-transform:uppercase}.commands{display:grid;gap:var(--space-2)}.commands section{display:grid;gap:var(--space-1)}.commands button{display:grid;gap:var(--space-1);min-height:var(--control-height-default);padding:var(--space-1) var(--space-2);border:0;border-radius:var(--radius-control);background:transparent;color:var(--text);text-align:left;cursor:pointer}.commands button:hover,.commands button:focus-visible{background:var(--surface-hover)}.commands small,.results span,.results small,.summary,.empty{color:var(--text-muted);font-size:var(--text-size-meta)}.global-search-entry__form{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:var(--space-2)}.global-search-entry input,.global-search-entry select,.global-search-entry form button,.search-secondary-action{border:1px solid var(--control-border);border-radius:var(--radius-control);padding:var(--space-2);background:var(--control-bg);color:var(--text);font:inherit}.global-search-entry select{min-width:7rem}.global-search-entry__submit{background:var(--accent)!important;border-color:var(--accent)!important;color:var(--canvas)!important;cursor:pointer}.global-search-entry__submit:disabled{cursor:not-allowed;opacity:.55}.results{display:grid;gap:var(--space-1);max-height:24rem;overflow:auto}.result-row{display:grid;grid-template-columns:auto minmax(0,1fr);align-items:start;gap:var(--space-1)}.result-row>input{margin-top:var(--space-2)}.result-row__open{display:grid;gap:var(--space-1);width:100%;padding:var(--space-2);border:0;border-radius:var(--radius-control);background:transparent;color:var(--text);text-align:left;cursor:pointer}.result-row__open:hover,.result-row__open:focus-visible{background:var(--surface-hover)}.compare-action{width:100%;border:1px solid var(--control-border);border-radius:var(--radius-control);padding:var(--space-2);background:var(--control-bg);color:var(--text);cursor:pointer}.compare-action:disabled{opacity:.55;cursor:not-allowed}.search-secondary-action{cursor:pointer;color:var(--text-soft)}.search-secondary-action:hover,.search-secondary-action:focus-visible{border-color:var(--control-border-active);color:var(--text)}.summary-row{display:flex;align-items:center;justify-content:space-between;gap:var(--space-2)}.summary-row .summary{margin:0}.comparison{display:grid;gap:var(--space-2);border-top:1px solid var(--border-subtle);padding-top:var(--space-2)}.comparison-item{display:grid;gap:var(--space-1);border:1px solid var(--border-subtle);padding:var(--space-2)}.comparison-item small,.comparison-item dt,.comparison-item dd{color:var(--text-muted);font-size:var(--text-size-meta)}.comparison-item dl{display:grid;grid-template-columns:auto 1fr;gap:var(--space-1) var(--space-2);margin:0}.comparison-item dd{margin:0;color:var(--text)}.error{color:var(--danger);font-size:var(--text-size-body)}
.shortcut-section{display:grid;gap:var(--space-2);border-top:1px solid var(--border-subtle);padding-top:var(--space-2)}.shortcut-section>summary{cursor:pointer;color:var(--text-muted);font-weight:var(--text-weight-semibold);list-style:none}.shortcut-section>summary::-webkit-details-marker{display:none}.shortcut-section>summary::after{content:"＋";float:right;color:var(--text-soft)}.shortcut-section[open]>summary::after{content:"−"}.saved-link{color:var(--text-muted);font-size:var(--text-size-meta)}
@media (max-width: 38rem){.global-search-entry__form{grid-template-columns:1fr auto}.global-search-entry select{grid-column:1}.global-search-entry__submit{grid-column:2;grid-row:2}.summary-row{align-items:flex-start;flex-direction:column}}
</style>
