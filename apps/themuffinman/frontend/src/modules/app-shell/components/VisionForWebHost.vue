...
const props = withDefaults(defineProps<{context: string; source: string; returnTo: string; persistent?: boolean}>(), {persistent: false})
...
<template>
  ...
  <section class="vision-web-host" aria-label="Vision assistant" data-context-source="contextual-workspace">
    ...
    <div v-if="open || props.persistent" id="vision-web-panel" class="vision-web-host__panel" :class="{'vision-web-host__panel--persistent': props.persistent}" role="dialog" aria-modal="false" aria-label="Ask Vision" aria-describedby="vision-web-hint">
      <header class="vision-web-host__header">
        <div><strong>Vision</strong><span>{{ props.context }}</span></div>
        <button v-if="!props.persistent" type="button" class="vision-web-host__close" aria-label="Close Vision" @click="close">×</button>
      </header>
