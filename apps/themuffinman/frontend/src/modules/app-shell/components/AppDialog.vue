...
<template>
  <Teleport to="body">
    <div v-if="open" class="app-dialog__backdrop" role="presentation" @click.self="emit('close')" @keydown.escape="emit('close')">
      <section class="app-dialog" role="dialog" aria-modal="true" :aria-label="title" aria-describedby="app-dialog-body">
        ...
        <div id="app-dialog-body" class="app-dialog__body"><slot /></div>
      </section>
    </div>
  </Teleport>
</template>
