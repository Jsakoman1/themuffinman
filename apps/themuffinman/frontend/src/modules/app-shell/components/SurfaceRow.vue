...
<template>
  <article class="surface-row" :data-collection-row-id="row.id" :class="[`surface-row--${density}`, {'surface-row--selected': selected, 'surface-row--previewed': previewed, 'surface-row--informational': !row.to}]" :tabindex="primaryAction === 'preview' || !row.to ? 0 : undefined" @keydown.enter.prevent="primaryAction === 'preview' && emit('open')" @keydown.p.prevent="primaryAction === 'preview' && emit('preview')">
