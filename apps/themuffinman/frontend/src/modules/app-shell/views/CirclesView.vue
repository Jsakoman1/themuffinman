...
import type {CircleContactDTO, CircleGroupResponseDTO, CircleRequestResponseDTO, CircleSearchResultDTO} from "../../../contracts/index.ts"
...
const connections = ref<CircleContactDTO[]>([])
...
const load = async () => {
  ...
  const [circleGroups, incoming, outgoingRequests, blockedUsers, acceptedConnections] = results
  ...
  if (acceptedConnections.status === "fulfilled") connections.value = acceptedConnections.value.items
  else loadWarnings.value.push("connections")
  if (loadWarnings.value.length === 5) error.value = "Could not load circles."
...
const decide = async (request: CircleRequestResponseDTO, accept: boolean) => { if (!accept && !await confirmAction(`Remove the request involving ${request.counterpartUsername}?`, "Remove request")) return; isActing.value = true; error.value = ""; try { if (accept) await userShellApi.acceptCircleRequest(request.id); else await userShellApi.deleteCircleRequest(request.id); feedback.value = accept ? `You are now connected with ${request.counterpartUsername}. Add them to a circle only if you want to share that group.` : "Request removed."; await load() } catch { error.value = "Could not update this request." } finally { isActing.value = false } }
...
const addToSelectedGroup = async (person: CircleContactDTO) => {
  if (!selectedGroup.value) return
  isActing.value = true; error.value = ""
  try { await userShellApi.addCircleMember(selectedGroup.value.id, person.userId); feedback.value = `${person.username} added to ${selectedGroup.value.name}.`; await load() }
  catch { error.value = "Could not add this person to the circle. Try again after refreshing the circle." }
  finally { isActing.value = false }
}
...
<template>
  ...
  <TaskSurface mode="workspace" label="Trust circles"><section class="circles" data-mental-model="people-groups-permissions" :aria-busy="isLoading || isActing || undefined">
    <FriendlyCollectionHeader eyebrow="Circles" :title="activeTab === 'people' ? 'Find people' : activeTab === 'requests' ? 'Connection requests' : 'Your circles'" :description="activeTab === 'people' ? 'Find someone you know, then send a connection request.' : activeTab === 'requests' ? 'Decide incoming invitations or withdraw requests you sent.' : 'Choose who shares a trusted space with you.'" tone="people"><template #actions><AppButton type="button" tone="primary" @click="createOpen = true">Create circle</AppButton></template></FriendlyCollectionHeader>
    ...
    <template v-else>
      <section v-if="activeTab === 'people'" class="circles__section" data-surface="people-organizer"><h2>Find people</h2><form class="circles__search" @submit.prevent="search"><AppFormField label="Search by name"><input v-model="searchQuery" placeholder="Search by name"></AppFormField><AppButton type="submit">Search people</AppButton></form><p class="circles__hint">Step one is only a connection request. Circles are a separate, optional sharing decision after the person accepts.</p><article v-for="person in results" :key="person.id" class="circles__row"><div><strong>{{ person.username }}</strong><span>{{ person.relationLabel || person.profileDescription }}</span></div><div class="circles__actions"><AppButton v-if="person.primaryAction?.enabled" tone="primary" type="button" :loading="isActing" @click="connect(person)">Send connection request</AppButton><AppButton tone="quiet" type="button" @click="block(person.id)">Block person</AppButton></div></article><p v-if="searchQuery && !results.length" class="circles__status">No people found. Try a different name.</p></section>
      <section v-if="activeTab === 'requests'" class="circles__section" data-surface="circle-requests"><h2>Connection requests</h2><section class="circles__request-outcome" aria-label="What accepting changes"><p class="circles__eyebrow">What happens next</p><strong>Accepting makes a connection.</strong><span>It does not add anyone to a circle or grant access to your location or private profile.</span></section><p v-if="!requests.length && !outgoing.length" class="circles__status">No requests need your attention.</p><section v-if="requests.length"><h3>Waiting for your decision</h3><article v-for="request in requests" :key="request.id" class="circles__row"><div><strong>{{ request.requesterUsername }}</strong><span>{{ request.requestSummaryLabel }}</span></div><div class="circles__actions"><AppButton tone="primary" :loading="isActing" @click="decide(request, true)">Accept connection</AppButton><AppButton tone="quiet" :loading="isActing" @click="decide(request, false)">Decline request</AppButton></div></article></section><section v-if="outgoing.length"><h3>Waiting for their decision</h3><article v-for="request in outgoing" :key="request.id" class="circles__row"><div><strong>{{ request.counterpartUsername }}</strong><span>{{ request.requestSummaryLabel }}</span></div><AppButton tone="quiet" :loading="isActing" @click="decide(request, false)">Withdraw request</AppButton></article></section></section>
      <section v-if="activeTab === 'groups'" class="circles__section" data-surface="circle-groups"><h2>Circles you manage</h2><p class="circles__hint">Select one circle, then manage its members here. Connections are not added to a circle unless you choose them.</p><section v-if="groups.length === 0" class="circles__empty"><FriendlySummaryCard eyebrow="Start a trusted space" title="Create a circle for people who share something" description="A circle is optional. It helps you choose a trusted group for shared activities and visibility." icon="◎" tone="people" /><AppButton type="button" tone="primary" @click="createOpen = true">Create your first circle</AppButton></section><div v-else class="circles__circles-workspace"><div class="circles__circle-list"><article v-for="group in groups" :key="group.id" class="circles__row" :class="{ 'circles__row--selected': selectedGroupId === group.id }" @click="selectedGroupId = group.id"><form v-if="editingGroupId === group.id" class="circles__inline-edit" @submit.prevent="saveGroup"><input v-model="editingGroupName" required maxlength="120"><AppButton tone="primary" type="submit" :loading="isActing">Save name</AppButton><AppButton tone="quiet" type="button" @click="editingGroupId = null">Cancel</AppButton></form><div v-else><strong>{{ group.name }}</strong><span>{{ group.memberCount }} members · {{ group.memberPreviewLabel }}</span></div><div v-if="editingGroupId !== group.id" class="circles__actions"><AppButton type="button" @click.stop="beginEditGroup(group)">Edit circle</AppButton><AppButton tone="danger" :loading="isActing" @click.stop="archiveGroup(group)">Remove circle</AppButton><AppButton tone="quiet" :loading="isActing" @click.stop="leaveCircle(group.id)">Leave circle</AppButton></div></article></div><aside v-if="selectedGroup" class="circles__context" aria-label="Selected circle context" data-visibility-boundary="circle"><p class="circles__eyebrow">Selected circle</p><h2>{{ selectedGroup.name }}</h2><p>{{ selectedGroup.memberCount }} members</p><dl><div><dt>Sharing context</dt><dd>Circle-scoped features</dd></div><div><dt>What changes here</dt><dd>Membership and shared chat</dd></div></dl><div v-if="selectedGroup.members?.length" class="circles__context-members"><strong>Members</strong><span v-for="member in selectedGroup.members" :key="member.userId">{{ member.username }} <AppButton tone="danger" :loading="isActing" @click="removeMember(selectedGroup.id, member.userId, member.username)">Remove member</AppButton></span></div><section class="circles__add-members" aria-label="Add connected people"><strong>Add a connected person</strong><p>Choose a person to add to {{ selectedGroup.name }}. This only changes this circle.</p><div v-for="person in connections.filter(contact => !selectedGroup!.members?.some(member => member.userId === contact.userId))" :key="person.userId" class="circles__add-member"><span>{{ person.username }}</span><AppButton type="button" tone="secondary" :loading="isActing" @click="addToSelectedGroup(person)">Add to this circle</AppButton></div><p v-if="!connections.some(contact => !selectedGroup!.members?.some(member => member.userId === contact.userId))" class="circles__context-note">All of your connections are already in this circle, or you have no accepted connections yet.</p></section><div class="circles__club-actions"><AppButton tone="primary" :loading="isActing" @click="createClubChat(selectedGroup)">Create circle chat</AppButton><RouterLink to="/calendar" class="circles__calendar-link">Plan a circle event in Calendar</RouterLink></div><p class="circles__context-note">Exact location and private profile fields are never implied by membership. The server decides every module action.</p></aside><aside v-else class="circles__context circles__context--empty" aria-label="Circle context"><p class="circles__eyebrow">Circle context</p><h2>Select a circle</h2><p>Select a circle to inspect members and add or remove people here.</p></aside></div></section>
...
<style scoped>
.circles__circles-workspace,.circles__request-outcome{border-top-color:var(--orientation-line)}
...
.circles__empty{display:grid;justify-items:start;gap:var(--space-3);max-width:36rem}
.circles__request-outcome{display:grid;gap:var(--space-1);max-width:46rem;padding:var(--space-3);border:1px solid var(--border-subtle);border-radius:var(--radius-surface);background:var(--surface-raised)}.circles__request-outcome strong{color:var(--text)}.circles__request-outcome span{color:var(--text-muted);font-size:var(--text-size-meta)}
...
.circles__add-members{display:grid;gap:var(--space-2);padding-top:var(--space-2);border-top:1px solid var(--border-subtle)}.circles__add-members>p{color:var(--text-muted);font-size:var(--text-size-meta)}.circles__add-member{display:flex;align-items:center;justify-content:space-between;gap:var(--space-2)}.circles__add-member span{color:var(--text);font-size:var(--text-size-meta)}
