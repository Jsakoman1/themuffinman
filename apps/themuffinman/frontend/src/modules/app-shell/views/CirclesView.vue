...
const archiveGroup = async (group: CircleGroupResponseDTO) => { if (!window.confirm(`Remove the circle “${group.name}”?`)) return; isActing.value = true; error.value = ""; try { await userShellApi.deleteCircleGroup(group.id); feedback.value = "Circle removed."; await load() } catch { error.value = "Could not remove this circle." } finally { isActing.value = false } }
...
const leaveCircle = async (circleId: number) => { if (!window.confirm("Leave this circle?")) return; isActing.value = true; error.value = ""; try { await userShellApi.leaveCircle(circleId); feedback.value = "Circle membership removed."; await load() } catch { error.value = "Could not leave this circle. Owners must remove the circle instead." } finally { isActing.value = false } }
const removeMember = async (circleId: number, userId: number, username: string) => { if (!window.confirm(`Remove ${username} from this circle?`)) return; isActing.value = true; error.value = ""; try { await userShellApi.removeCircleMember(circleId, userId); feedback.value = `${username} removed from the circle.`; await load() } catch { error.value = "Could not remove this member." } finally { isActing.value = false } }
onMounted(() => void load())
</script>
