...
const userShellApi = {getThingPreview: thingsApi.getPreview, getListingsForScope: thingsApi.getListingsForScope, getThingOwnerBorrowRequests: thingsApi.getOwnerBorrowRequests, getMyThingBorrowRequests: thingsApi.getMyBorrowRequests, createThingListing: (request: ThingListingRequestDTO) => thingsApi.createQuickListing(request.title, request.description), requestThingBorrow: thingsApi.requestBorrow, cancelThingBorrow: thingsApi.cancelBorrow, decideThingBorrow: thingsApi.decideBorrow, returnThingBorrow: thingsApi.returnBorrow, updateThingListing: thingsApi.updateListing, archiveThingListing: thingsApi.archiveListing}
...
<template>
  ...
  <TaskSurface mode="inspect" label="Things sharing"><section class="things-surface" :aria-busy="isLoading || isSaving || undefined">
    ...
    <CollectionToolbar :title="title" :count="requestsOnly ? borrowerRequests.length : listings.length" :busy="isLoading"><template #filters><nav v-if="!requestsOnly" class="things-surface__scope" aria-label="Thing view"><AppButton type="button" :tone="!mine ? 'primary' : 'secondary'" @click="router.replace({query: {...route.query, scope: undefined}})">Discover</AppButton><AppButton type="button" :tone="mine ? 'primary' : 'secondary'" @click="router.replace({query: {...route.query, scope: 'mine'}})">My things</AppButton><form class="things-surface__need-search" @submit.prevent="searchNeeds"><input v-model="needQuery" aria-label="What do you need?" placeholder="What do you need?"><AppButton type="submit">Search</AppButton></form></nav></template><template #actions><AppButton v-if="!requestsOnly" type="button" tone="primary" @click="form = {title: '', description: '', conditionNote: '', available: true}">List a thing</AppButton></template></CollectionToolbar>
    <p class="things-surface__cue" data-surface="listing-request-context">Start with what you need. Availability, owner context, and borrow actions come from the server.</p>
...
.things-surface__need-search { display:flex; gap:var(--space-1); align-items:center; }
.things-surface__need-search input { min-width:12rem; border:1px solid var(--control-border); border-radius:var(--radius-control); padding:var(--space-1) var(--space-2); background:var(--control-bg); color:var(--control-ink); font:inherit; }
</style>
