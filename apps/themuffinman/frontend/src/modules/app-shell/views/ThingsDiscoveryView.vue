...
const surface = getAppSurfaceConfig("things")
...
const requestOutcome = computed(() => {
  ...
  if (selectedListing.value.allowedActions.includes("REQUEST_BORROW")) return "Sending a request asks the owner to approve your borrowing details."
  if (selectedListing.value.allowedActions.includes("CANCEL_BORROW_REQUEST")) return "Cancelling withdraws your pending request before the owner decides."
  return "This listing is not available for a new borrow request right now."
})
...
const decideBorrow = async (requestId: number, approve: boolean) => { if (!approve && !await confirmAction("This tells the borrower their request was not approved.", "Decline request")) return; await execute(async () => { await userShellApi.decideThingBorrow(requestId, approve); feedback.value = approve ? "Borrow request approved. The thing is now unavailable until it is returned." : "Borrow request declined."; await load() }, "Could not update this request.") }
...
const openDetail = () => {
  if (!selectedListing.value) return
  void router.push({path: `/things/${selectedListing.value.id}`, query: {returnTo: route.fullPath}})
}
...
<template>
  ...
  <TaskSurface mode="inspect" label="Things sharing"><section class="things-surface" data-preview-model="shared-adjacent-preview" data-mental-model="browse-preview-wishlist-request" :aria-busy="isLoading || isSaving || undefined">
    <SurfaceHeader :config="surface" :title="title" :description="requestsOnly ? 'Track requests you sent and see what you need to do next.' : mine ? 'Manage what you share and decide who can borrow it.' : 'Find things shared by people in your circles.'" />
    ...
    <div v-else-if="!requestsOnly" class="things-workspace" :class="{'things-workspace--preview': selectedListing}"><div class="listing-grid"><SurfaceRow v-for="listing in listings" :key="listing.id" :row="{id: String(listing.id), title: listing.title, description: `${listing.ownerUsername} · ${listing.description || 'No description yet.'}`, meta: listing.conditionNote || 'Condition not specified', to: {path: `/things/${listing.id}`, query: {returnTo: route.fullPath}}}" primary-action="preview" :selected="viewState.selectedId === listing.id" @preview="viewState.selectedId = listing.id"><template #actions><AppButton v-if="listing.allowedActions.includes('EDIT')" type="button" tone="secondary" @click.stop="editListing(listing)">Edit listing</AppButton><AppButton v-if="listing.allowedActions.includes('ARCHIVE')" type="button" tone="danger" @click.stop="archiveListing(listing)">Archive listing</AppButton><AppButton v-if="listing.allowedActions.includes('REQUEST_BORROW')" type="button" tone="primary" @click.stop="borrowId = listing.id">Request to borrow</AppButton><AppButton v-if="listing.allowedActions.includes('CANCEL_BORROW_REQUEST') && listing.myPendingRequestId" type="button" tone="secondary" @click.stop="cancelBorrow(listing.myPendingRequestId)">Withdraw request</AppButton></template></SurfaceRow></div><ObjectPreviewPanel v-if="selectedListing" :open="true" :title="selectedListing.title" subtitle="Selected thing" :action-outcome="requestOutcome" @close="viewState.selectedId = null" @open-detail="openDetail"><p>{{ selectedListing.description || 'No description provided.' }}</p><dl><div><dt>Owner</dt><dd>{{ selectedListing.ownerUsername }}</dd></div><div><dt>Availability</dt><dd>{{ selectedListing.availabilityLabel || (selectedListing.available ? 'Available' : 'Unavailable') }}</dd></div><div v-if="selectedListing.conditionNote"><dt>Condition</dt><dd>{{ selectedListing.conditionNote }}</dd></div></dl></ObjectPreviewPanel></div>
    <section v-if="!mine && borrowerRequests.length" class="requests"><h2>Your active borrowing</h2><article v-for="request in borrowerRequests" :key="request.id" class="request"><div><strong>{{ request.status }}</strong><small>{{ request.message || 'Waiting for the owner’s decision.' }}</small></div><div class="request__actions"><AppButton v-if="request.allowedActions.includes('RETURN_BORROWED_THING')" type="button" tone="primary" @click="returnBorrow(request.id)">Mark thing returned</AppButton><AppButton v-if="request.allowedActions.includes('CANCEL_BORROW_REQUEST')" type="button" tone="secondary" @click="cancelBorrow(request.id)">Withdraw request</AppButton></div></article></section>
    <section v-if="mine && ownerRequests.length" class="requests"><h2>Needs your decision</h2><p class="requests__hint">Approving makes the thing unavailable until the borrower marks it returned.</p><article v-for="request in ownerRequests" :key="request.id" class="request"><div><strong>{{ request.borrowerUsername }} · {{ request.status }}</strong><small>{{ request.message || 'No message from the borrower.' }}</small></div><div class="request__actions"><AppButton v-if="request.allowedActions.includes('APPROVE_BORROW_REQUEST')" type="button" tone="primary" :loading="isSaving" @click="decideBorrow(request.id, true)">Approve request</AppButton><AppButton v-if="request.allowedActions.includes('DECLINE_BORROW_REQUEST')" type="button" tone="danger" :loading="isSaving" @click="decideBorrow(request.id, false)">Decline request</AppButton></div></article></section>
