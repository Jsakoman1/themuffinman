...
<template>
  ...
  <TaskSurface mode="inspect" label="Things sharing"><section class="things-surface" data-preview-model="shared-adjacent-preview" data-mental-model="browse-preview-wishlist-request" :aria-busy="isLoading || isSaving || undefined">
    <SurfaceHeader :config="surface" :title="title" :description="requestsOnly ? 'Review borrow requests connected to your things.' : mine ? 'Manage your listings and incoming borrow requests.' : 'Find things shared by people in your circles.'" />
    <CollectionToolbar :title="title" :count="requestsOnly ? borrowerRequests.length : listings.length" :busy="isLoading"><template #filters><form v-if="!requestsOnly" class="things-surface__need-search" aria-label="Search things" @submit.prevent="searchNeeds"><input v-model="needQuery" aria-label="What do you need?" placeholder="What do you need?"><AppButton type="submit">Search</AppButton></form></template><template #actions><AppButton v-if="!requestsOnly" type="button" tone="primary" @click="form = {title: '', description: '', conditionNote: '', available: true}">List a thing</AppButton></template></CollectionToolbar>
    <p class="things-surface__cue" data-surface="listing-request-context">Availability and borrow rules are shown when you select a thing.</p>
