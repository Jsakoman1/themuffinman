...
<template>
  <section class="offerings-surface">
    ...
    <div v-else class="offerings-surface__workspace"><div class="offerings-surface__list"><SurfaceRow v-for="offering in offerings" :key="offering.id" :row="{id: String(offering.id), title: offering.title, description: offering.summary || 'No summary', badge: offering.active ? 'Active' : 'Inactive', meta: `${offering.basePriceAmount} ${offering.basePriceCurrency} · ${offering.defaultDurationMinutes} min`}" :selected="selectedOfferingId === offering.id" @click="selectedOfferingId = offering.id"><template #actions><AppButton type="button" tone="secondary" @click.stop="beginEdit(offering)">Edit</AppButton><AppButton type="button" tone="danger" @click.stop="remove(offering)">Delete</AppButton></template></SurfaceRow></div><aside v-if="selectedOffering" class="offerings-surface__preview" aria-label="Offering context"><p class="offerings-surface__eyebrow">Offering context</p><h2>{{ selectedOffering.title }}</h2><p>{{ selectedOffering.summary || "No summary provided." }}</p><dl><div><dt>Status</dt><dd>{{ selectedOffering.active ? "Active" : "Inactive" }}</dd></div><div><dt>Price</dt><dd>{{ selectedOffering.basePriceAmount }} {{ selectedOffering.basePriceCurrency }}</dd></div><div><dt>Duration</dt><dd>{{ selectedOffering.defaultDurationMinutes }} min</dd></div><div><dt>Booking mode</dt><dd>{{ selectedOffering.bookingMode }}</dd></div></dl><p v-if="selectedOffering.description" class="offerings-surface__preview-note">{{ selectedOffering.description }}</p><div class="offerings-surface__preview-actions"><AppButton type="button" tone="secondary" @click="beginEdit(selectedOffering)">Edit offering</AppButton><AppButton type="button" tone="danger" @click="remove(selectedOffering)">Delete offering</AppButton></div></aside><aside v-else class="offerings-surface__preview offerings-surface__preview--empty" aria-label="Offering context"><p class="offerings-surface__eyebrow">Offering context</p><h2>Select an offering</h2><p>Pricing and booking details appear here when you select an offering.</p></aside></div>
