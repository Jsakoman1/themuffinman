...
const load = async () => { isLoading.value = true; error.value = ""; profile.value = null; try { profile.value = await userShellApi.getCurrentProfileView(userId.value) } catch { error.value = "Could not load this profile context." } finally { isLoading.value = false } }
