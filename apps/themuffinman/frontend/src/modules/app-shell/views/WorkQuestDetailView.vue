...
const save = async () => {
  ...
  try { await userShellApi.updateQuest(questId.value, {...form.value, scheduledAt: scheduledAtInput.value ? new Date(scheduledAtInput.value).toISOString() : null, endsAt: endsAtInput.value ? new Date(endsAtInput.value).toISOString() : null}); feedback.value = "Quest updated."; editing.value = false; await load() } catch (requestError) { error.value = isStaleResourceError(requestError) ? "This quest changed elsewhere. Reloaded the latest version; review your edits before saving again." : userShellApi.actionFailureMessage("Could not update this quest.", requestError); if (isStaleResourceError(requestError)) await load() } finally { isSaving.value = false }
}
...
const changeLifecycle = async (action: "ASSIGN" | "REOPEN") => {
  ...
  catch (requestError) { error.value = isStaleResourceError(requestError) ? "This quest changed elsewhere. Reloaded the latest version; try the action again." : "This quest action could not be completed."; if (isStaleResourceError(requestError)) await load() }
...
<template>
  <section class="quest-detail">
    ...
    <section v-if="detail" class="quest-detail__guidance" aria-label="Quest fit and progress"><strong>Decision guide</strong><span>Compatibility: review requirements and missing information before applying.</span><span>Application progress: {{ can('APPLY') ? 'Ready to apply' : 'Application action is not available in this state' }}</span><span>Reward/time: confirm the server-calculated award and expected schedule before committing.</span><span>Owner/applicant context: actions and visibility are limited by backend permissions.</span></section>
    ...
    <template v-else-if="detail">
      ...
      <AppDialog :open="applicationOpen" title="Apply to this quest" @close="applicationOpen = false"><form class="quest-detail__application" @submit.prevent="submitApplication"><AppFormField label="Message" hint="Tell the owner why you are a good fit." :required="applicationRules?.messageRequired"><textarea v-model="applicationMessage" :required="applicationRules?.messageRequired" maxlength="2000" placeholder="Tell the owner why you are a good fit."></textarea></AppFormField><AppFormField v-if="applicationRules?.proposedPriceRequired" label="Offer" hint="Include your proposed price." required><input v-model.number="applicationPrice" type="number" :min="applicationRules.minimumProposedPrice ?? undefined" step="0.01" required></AppFormField><p class="quest-detail__application-help">{{ applicationRules?.proposedPriceRequired ? "The server validates the proposed price against quest rules." : "This quest does not require a price." }}</p><AppFormFooter><template #secondary><AppButton type="button" tone="secondary" @click="applicationOpen = false">Cancel</AppButton></template><template #primary><AppButton type="submit" tone="primary" :loading="isSaving">{{ isSaving ? "Sending" : "Send application" }}</AppButton></template></AppFormFooter></form></AppDialog>
...
.quest-detail__guidance{display:grid;gap:var(--space-1);padding:var(--space-3);border:1px solid var(--border-subtle);border-radius:var(--radius-surface);background:var(--surface-raised);color:var(--text-muted)}.quest-detail__guidance strong{color:var(--text)}.quest-detail .app-button{width:100%;min-height:var(--control-height-default);border-radius:var(--radius-control);padding:var(--space-1) var(--space-3);background:var(--control-bg);color:var(--control-ink)}.quest-detail__header .app-button{width:auto}.quest-detail .app-button--primary{border-color:var(--accent);background:var(--accent);color:var(--canvas)}.quest-detail .app-button--danger{color:var(--danger)}
</style>
