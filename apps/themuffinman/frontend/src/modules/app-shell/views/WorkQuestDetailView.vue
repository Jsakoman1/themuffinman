...
<template>
  <section class="quest-detail">
    <DetailSurfaceHeader eyebrow="Work / Quest" :title="detail?.quest.title || 'Quest'" back-to="/work/quests" back-label="Back to quests"><template #actions><RouterLink v-if="returnToVision" :to="returnToVision" class="quest-detail__return">Back to Vision</RouterLink><AppButton type="button" tone="secondary" :aria-expanded="utilityOpen" aria-controls="quest-detail-actions" @click="utilityOpen = !utilityOpen">{{ utilityOpen ? "Hide actions" : "Show actions" }}</AppButton></template></DetailSurfaceHeader>
