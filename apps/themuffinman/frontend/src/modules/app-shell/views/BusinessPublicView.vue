...
const selectOffering = async (id: number) => { selectedOfferingId.value = id; currentStep.value = 1; demand.value = {}; selectedOptions.value = {}; quantity.value = 1; quote.value = null; availability.value = []; selectedSlot.value = ""; bookingDate.value = businessToday(); bookingError.value = ""; schema.value = await userShellApi.getPublicServiceSchema(String(route.params.slug), id) }
const resetFlow = () => { selectedOfferingId.value = null; currentStep.value = 0; schema.value = null; quote.value = null; availability.value = []; demand.value = {}; selectedOptions.value = {}; selectedSlot.value = ""; bookingDate.value = ""; customerNote.value = ""; quantity.value = 1 }
        ...
        <section v-else class="booking-flow native-group">
          ...
          <form @submit.prevent="currentStep === 3 ? book() : goNext()">
            ...
            <section v-else-if="currentStep === 2" class="booking-panel">
              ...
              <AppFormField label="Date" required :hint="`Times are shown in ${page.timezone}.`"><input v-model="bookingDate" type="date" required @change="loadAvailabilityForDate"></AppFormField>
