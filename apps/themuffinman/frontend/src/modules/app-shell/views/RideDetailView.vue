...
<template>
  <section class="ride-detail">
    ...
    <template v-else-if="detail">
      ...
      <section class="ride-detail__trust" data-surface="suggested-ride-trust" aria-label="Route and trust summary"><strong>Route and trust</strong><span>Suggested from consented commute preferences; joining is always explicit.</span><span>Pickup flexibility: confirm with the driver before joining.</span><span>Seat availability: {{ detail.joinedSeats }} / {{ detail.seats }}</span><span>Visibility: {{ detail.visibleCircleNames.length ? detail.visibleCircleNames.join(', ') : 'Viewer-scoped' }}</span><span>Cancellation impact: action availability is controlled by the ride state.</span></section>
    </template>
  </section>
</template>
