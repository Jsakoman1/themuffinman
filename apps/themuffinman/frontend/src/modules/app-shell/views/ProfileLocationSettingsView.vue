...
const chooseLocationMode = (value: UserLocationMode) => { mode.value = value }
...
<template>
  <!-- UX simplification: profile editing is a focused workspace with explicit save state and progressive disclosure. -->
  <TaskSurface mode="act" label="Profile and location settings"><section class="location-settings" data-settings-model="grouped-inline-edit" data-edit-model="inline-save-cancel" :aria-busy="isLoading || isSaving || isGalleryActing || isAppearanceSaving || undefined">
    <header data-settings-model="group-summary-focused-pane"><p class="eyebrow">Profile settings</p><h1>Profile and location</h1><p class="intro">Manage your profile, appearance, location, and privacy.</p></header>
    ...
    <form aria-label="Profile and location settings" @submit.prevent="save">
      ...
      <details class="settings-pane" id="profile-gallery"><summary><span><strong>Photo gallery</strong><small>Optional images shown on your profile</small></span><span class="settings-pane__value">{{ gallery.length ? `${gallery.length} photo${gallery.length === 1 ? "" : "s"}` : "Not configured" }}</span></summary><fieldset class="profile-gallery"><legend class="sr-only">Photo gallery</legend><p class="profile-gallery__intro">Optional photos for your profile. These are separate from your profile picture.</p><div class="profile-gallery__form"><AppFormField label="Image URL"><input v-model="galleryUrl" type="url" placeholder="https://…" aria-label="Image URL"></AppFormField><AppFormField label="Alt text" optional><input v-model="galleryAltText" maxlength="240" placeholder="Describe the photo" aria-label="Alt text"></AppFormField><AppButton type="button" tone="primary" :loading="isGalleryActing" @click="addGalleryImage">Add photo</AppButton></div><div v-if="gallery.length" class="profile-gallery__grid"><article v-for="image in gallery" :key="image.id" class="profile-gallery__item"><img :src="image.imageUrl" :alt="image.altText || 'Profile gallery photo'"><AppButton type="button" tone="danger" :loading="isGalleryActing" @click="removeGalleryImage(image)">Remove</AppButton></article></div><AppStatus v-else message="No profile gallery photos yet." /></fieldset></details>
      <details class="settings-pane" id="appearance-settings"><summary><span><strong>Appearance</strong><small>Theme and visual preferences</small></span><span class="settings-pane__value">{{ appearanceTheme }}</span></summary><fieldset class="appearance-settings"><legend class="sr-only">Appearance</legend><p class="profile-gallery__intro">VisionForWeb follows this Web setting; the future standalone Vision console will have its own shell.</p><AppFormField label="Theme"><select v-model="appearanceTheme" aria-label="Theme"><option value="SYSTEM">System</option><option value="DARK">Dark</option><option value="LIGHT">Light</option></select></AppFormField><AppButton type="button" tone="primary" :loading="isAppearanceSaving" @click="updateAppearance">Save appearance</AppButton></fieldset></details>
      <details class="settings-pane" id="location-fields"><summary><span><strong>Location and privacy</strong><small>Choose what, if anything, you share</small></span><span class="settings-pane__value">{{ mode === "OFF" ? "Not shared" : mode === "APPROXIMATE" ? "Approximate area" : "Exact address" }}</span></summary><fieldset><legend class="sr-only">Location and privacy</legend><section class="location-decision" aria-labelledby="location-decision-title"><div><p class="location-decision__eyebrow">Your choice</p><h2 id="location-decision-title">Share a location?</h2><p>Keep it off until a feature such as nearby work or rides makes it useful.</p></div><div class="location-decision__actions"><AppButton type="button" :tone="mode === 'OFF' ? 'primary' : 'secondary'" @click="chooseLocationMode('OFF')">Keep location off</AppButton><AppButton type="button" :tone="mode === 'APPROXIMATE' ? 'primary' : 'secondary'" @click="chooseLocationMode('APPROXIMATE')">Share approximate area</AppButton><AppButton type="button" :tone="mode === 'EXACT' ? 'primary' : 'secondary'" @click="chooseLocationMode('EXACT')">Share exact address</AppButton></div></section>
      <template v-if="mode !== 'OFF'"><p class="location-decision__next">{{ mode === 'EXACT' ? 'First decide who can see the exact address. Then choose the place to share.' : 'Choose a place to share. Your exact address stays hidden.' }}</p>
      <AppFormField v-if="mode === 'EXACT'" label="Who can see the exact address" hint="This is the only people-sharing choice required before you choose a place."><select ref="exactVisibilityInput" v-model="exactVisibilityScope" aria-label="Who can see the exact address"><option value="NOBODY">Only me</option><option value="EVERYONE">Everyone</option><option value="CIRCLES">Selected circles</option><option value="USERS">Selected people</option></select></AppFormField>
      ...
      <div v-if="candidate" class="selected"><strong>Resolved place</strong><span>{{ candidate.label }}</span><small>{{ candidate.locality || candidate.country || "Ready to save" }}</small><AppButton type="button" tone="quiet" @click="clearCandidate">Choose a different place</AppButton></div><details class="location-advanced"><summary>Fine-tune nearby search</summary><AppFormField label="Default search radius (km)"><input v-model.number="radius" type="number" min="1" max="200"></AppFormField></details></template>
      </fieldset></details><AppFormFooter sticky><template #secondary><AppButton type="button" tone="secondary" :disabled="isSaving || isLoading" @click="void load()">Reset unsaved changes</AppButton><span>Location visibility is enforced by the backend.</span></template><template #primary><AppButton type="submit" tone="primary" :loading="isSaving">{{ isSaving ? "Saving…" : "Save profile and location" }}</AppButton></template></AppFormFooter>
    </form>
...
.settings-guidance{margin:0;color:var(--text-muted);font-size:var(--text-size-meta)}.settings-pane{border-top:1px solid var(--native-separator);padding-top:var(--space-1)}.settings-pane summary{display:flex;align-items:center;justify-content:space-between;gap:var(--space-3);min-height:3.5rem;padding:var(--space-2) 0;color:var(--text);cursor:pointer;list-style:none}.settings-pane summary::-webkit-details-marker{display:none}.settings-pane summary::after{content:"⌄";color:var(--text-soft);font-size:1.1rem;transition:transform 160ms ease}.settings-pane[open] summary::after{transform:rotate(180deg)}.settings-pane summary span:first-child{display:grid;gap:.2rem}.settings-pane summary strong{font-size:var(--text-size-body);font-weight:var(--text-weight-semibold)}.settings-pane summary small{color:var(--text-muted);font-size:var(--text-size-meta)}.settings-pane__value{color:var(--text-muted);font-size:var(--text-size-meta);text-align:right}.settings-pane fieldset{padding:0 0 var(--space-3)}
...
.location-decision{display:grid;gap:var(--space-3);padding:var(--space-3);border:1px solid var(--border-subtle);border-radius:var(--radius-surface);background:var(--surface-raised)}.location-decision h2{margin:var(--space-1) 0;color:var(--text);font-size:var(--text-size-title)}.location-decision p{margin:0;color:var(--text-muted)}.location-decision__eyebrow{color:var(--text-soft)!important;font-size:var(--text-size-label);font-weight:var(--text-weight-semibold);letter-spacing:var(--tracking-label);text-transform:uppercase}.location-decision__actions{display:flex;flex-wrap:wrap;gap:var(--space-2)}.location-decision__next{margin:0;color:var(--text-muted)}.location-advanced{display:grid;gap:var(--space-2);padding-top:var(--space-2)}.location-advanced summary{color:var(--text-muted);cursor:pointer;font-weight:var(--text-weight-semibold)}.location-advanced[open] summary{margin-bottom:var(--space-2)}.location-settings :deep(.app-form-footer--sticky) { position: static; margin-inline: 0; margin-bottom: 0; }
