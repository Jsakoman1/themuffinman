...
const actionHint = computed(() => view.value?.pendingApplicationCount ? "Review pending applicants first." : "No pending applications require a decision.")
const previewApplication = computed(() => orderedApplications.value.find(item => item.id === selectedApplicationId.value) ?? null)
const actionOutcome = (application: QuestApplicationsViewDTO["visibleApplications"][number]) => {
  if (application.allowedActions.includes("APPROVE")) return "Approving tells this person they were selected and assigns them to this work."
  if (application.allowedActions.includes("DECLINE")) return "Declining tells this person their application was not selected."
  if (application.allowedActions.includes("RELEASE_WORKER")) return "Releasing ends this worker's assignment and reopens the available slot."
  if (application.allowedActions.includes("REPLACE_WORKER")) return "Replacing this worker assigns the available slot to another pending applicant."
  return "There is no decision available for this application right now."
}
...
const decide = async (applicationId: number, decision: "approve" | "decline") => { if (decision === "decline" && !await confirmAction("This tells the applicant they were not selected.", "Decline applicant")) return; isWorking.value = true; error.value = ""; feedback.value = ""; try { await userShellApi.decideQuestApplication(questId(), applicationId, decision); feedback.value = decision === "approve" ? "Applicant approved and assigned to this work." : "Applicant declined."; await load() } catch (cause) { error.value = userShellApi.actionFailureMessage("Could not update this application.", cause) } finally { isWorking.value = false } }
...
<template>
  <section class="quest-applications">
    ...
    <template v-else-if="view">
      ...
      <div class="quest-applications__summary"><span>{{ view.pendingApplicationCount }} pending · {{ view.hiddenApplicationsCount }} hidden</span><span class="quest-applications__hint">{{ actionHint }}</span><span class="quest-applications__hint">Open an applicant to review message, proposed price, and submission time.</span><AppButton v-if="view.canRevealHiddenApplications" type="button" tone="quiet" @click="load(!view.showingAllApplications)">{{ view.revealLabel }}</AppButton></div>
      <AppStatus v-if="orderedApplications.length === 0" message="No applications to review." /><div v-else class="quest-applications__workspace"><div class="quest-applications__list"><SurfaceRow v-for="application in orderedApplications" :key="application.id" :row="{id: String(application.id), title: application.applicantUsername, description: application.message || 'No message', badge: application.presentation.statusLabel, meta: `${application.proposedPrice == null ? 'No proposed price' : `${application.proposedPrice} €`} · ${formatDateTime(application.createdAt, 'Unknown date')}`}" :selected="selectedApplicationId === application.id" :previewed="previewApplication?.id === application.id" @click="selectedApplicationId = application.id"><template #actions><AppButton v-if="application.allowedActions.includes('APPROVE')" type="button" tone="primary" :loading="isWorking" @click.stop="decide(application.id, 'approve')">Approve applicant</AppButton><AppButton v-if="application.allowedActions.includes('DECLINE')" type="button" tone="danger" :loading="isWorking" @click.stop="decide(application.id, 'decline')">Decline applicant</AppButton><AppButton v-if="application.allowedActions.includes('RELEASE_WORKER')" type="button" tone="danger" :loading="isWorking" @click.stop="release(application.id)">Release worker</AppButton><AppButton v-if="application.allowedActions.includes('REPLACE_WORKER') && pendingApplications.length" type="button" tone="secondary" :loading="isWorking" @click.stop="replacingApplicationId = application.id">Replace worker</AppButton></template></SurfaceRow></div><ObjectPreviewPanel :title="previewApplication?.applicantUsername ?? 'Applicant'" subtitle="Application review" :open="previewApplication !== null" detail-label="Back to quest" :action-outcome="previewApplication ? actionOutcome(previewApplication) : ''" @close="selectedApplicationId = null" @open-detail="router.push(`/work/quests/${questId()}`)"><p>{{ previewApplication?.message || 'No application message.' }}</p><p v-if="previewApplication">{{ previewApplication.presentation.statusLabel }}</p><p v-if="previewApplication">{{ previewApplication.proposedPrice == null ? 'No proposed price' : `Proposed price: ${previewApplication.proposedPrice} €` }}</p><p v-if="previewApplication">Submitted {{ formatDateTime(previewApplication.createdAt, 'Unknown date') }}</p></ObjectPreviewPanel></div>
