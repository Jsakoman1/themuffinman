...
const saveEdit = async (application: QuestApplicationResponseDTO) => { isLoadingMore.value = true; error.value = ""; feedback.value = ""; try { await userShellApi.updateMyApplication(application.questId, {message: editMessage.value, proposedPrice: editPrice.value}); editingId.value = null; feedback.value = "Application updated."; await load() } catch (cause) { error.value = userShellApi.actionFailureMessage("Could not update this application. Review the fields and try again.", cause) } finally { isLoadingMore.value = false } }
...
<template>
  <section class="applications-surface">
    <SurfaceHeader :config="surface" description="Review the applications you submitted and the next actions available to you." />
    <CollectionToolbar title="My applications" :count="totalItems" :busy="isLoading" /><p class="applications-surface__contract-note">Edit remains available while the application is pending; the server decides which fields and actions are still allowed.</p>
...
<style scoped>
.applications-surface{display:grid;gap:var(--space-3)}.applications-surface__workspace{display:grid;grid-template-columns:minmax(0,1fr) minmax(18rem,var(--detail-rail-width));overflow:hidden;border:1px solid var(--border-subtle);border-radius:var(--radius-surface);background:var(--surface-base)}.applications-surface__list{min-width:0}.applications-surface__list :deep(.surface-row:last-child){border-bottom:0}.applications-surface__withdraw{color:var(--danger)}.applications-surface__edit{display:grid;gap:var(--space-3)}.applications-surface__edit textarea,.applications-surface__edit input{width:100%;box-sizing:border-box;border:1px solid var(--control-border);border-radius:var(--radius-control);padding:var(--space-2);background:var(--control-bg);color:var(--text);font:inherit}.applications-surface__edit textarea{min-height:7rem}@media(max-width:980px){.applications-surface__workspace{grid-template-columns:1fr}}
