...
// Application context follows the submitted application rather than a side preview.
...
const load = async () => {
  ...
  error.value = ""
  try { detail.value = await userShellApi.getApplicationDetail(applicationId.value) }
  catch { error.value = "Could not load this application." }
  finally { isLoading.value = false }
}

watch(applicationId, () => { detail.value = null; void load() })
onMounted(() => void load())
</script>
<style scoped>
.application-detail__status-badge{background:var(--orientation-action-bg)}
...
.application-detail__meta { line-height: 1.5; }
.application-detail__status-badge { background: var(--surface-selected); color: var(--text); }

.application-detail__card[aria-label="Application narrative and activity"]{border-top:2px solid var(--accent)}
</style>

<template>
  <section class="application-detail"><AppStatus v-if="isLoading" message="Loading application." busy /><AppStatus v-else-if="error" :message="error" tone="error" retry @retry="load" /><DetailSurface v-else-if="detail" :title="detail.application.questTitle" utility-label="Quest context"><template #header><DetailSurfaceHeader eyebrow="Work / Application" :title="detail.application.questTitle || 'Application'" back-to="/work/applications" back-label="Back to applications" /></template><template #default><div class="application-detail__status-badge">{{ detail.application.presentation.statusLabel }}</div><p class="application-detail__message">{{ detail.application.message || "No application message." }}</p><dl><div><dt>Offer</dt><dd>{{ detail.application.proposedPrice ?? "Not specified" }}{{ detail.application.proposedPrice == null ? "" : " €" }}</dd></div><div><dt>Submitted</dt><dd>{{ formatDate(detail.application.createdAt) }}</dd></div><div><dt>Quest status</dt><dd>{{ detail.application.presentation.questStatusLabel }}</dd></div></dl></template><template #utility><DetailUtilityRail title="Quest context"><p>{{ detail.quest.description }}</p><dl><div><dt>Award</dt><dd>{{ detail.quest.awardAmount }} €</dd></div><div><dt>Location</dt><dd>{{ detail.quest.locationLabel || detail.quest.locationLocality || "Profile or flexible location" }}</dd></div><div><dt>When</dt><dd>{{ formatDate(detail.quest.scheduledAt) }}</dd></div></dl><RouterLink :to="`/work/quests/${detail.quest.id}`">Open quest</RouterLink><p class="application-detail__meta">Posted by {{ detail.quest.creatorUsername }}</p></DetailUtilityRail></template></DetailSurface></section>
</template>
