  ...
  <section class="application-detail">
    ...
    <template v-else-if="detail">
      <div class="application-detail__grid">
        <main class="application-detail__card"><div class="application-detail__status-badge">{{ detail.application.presentation.statusLabel }}</div><p class="application-detail__message">{{ detail.application.message || "No application message." }}</p><dl><div><dt>Offer</dt><dd>{{ detail.application.proposedPrice ?? "Not specified" }}{{ detail.application.proposedPrice == null ? "" : " €" }}</dd></div><div><dt>Submitted</dt><dd>{{ formatDate(detail.application.createdAt) }}</dd></div><div><dt>Quest status</dt><dd>{{ detail.application.presentation.questStatusLabel }}</dd></div></dl></main>
        <aside class="application-detail__card application-detail__context"><h2>Quest context</h2><p>{{ detail.quest.description }}</p><RouterLink :to="`/work/quests/${detail.quest.id}`">Open quest</RouterLink><p class="application-detail__meta">Posted by {{ detail.quest.creatorUsername }} · {{ formatDate(detail.quest.scheduledAt) }}</p></aside>
      </div>
    </template>
  </section>
</template>
...
<style scoped>
.application-detail{display:grid;gap:1rem}.application-detail__header{display:flex;justify-content:space-between;align-items:end;gap:1rem}.application-detail__eyebrow{margin:0 0 .3rem;color:rgba(23,34,26,.55);font-size:.76rem;font-weight:650;letter-spacing:.08em;text-transform:uppercase}h1{margin:0;font-size:clamp(1.55rem,2.5vw,2.3rem);letter-spacing:-.075em}.application-detail__header a,.application-detail__context a{font-size:.84rem}.application-detail__grid{display:grid;grid-template-columns:minmax(0,1.25fr) minmax(14rem,.75fr);gap:1rem}.application-detail__card{border:1px solid rgba(23,34,26,.08);border-radius:1rem;background:rgba(255,255,255,.62);padding:1rem}.application-detail__status-badge{display:inline-flex;border-radius:999px;background:#e8efe7;padding:.4rem .7rem;font-size:.8rem;font-weight:650}.application-detail__message{white-space:pre-wrap;line-height:1.55}.application-detail__card dl{display:grid;gap:.7rem;margin:1.4rem 0 0}.application-detail__card dl div{display:flex;justify-content:space-between;gap:1rem;border-top:1px solid rgba(23,34,26,.08);padding-top:.7rem}.application-detail__card dt,.application-detail__meta{color:rgba(23,34,26,.58);font-size:.82rem}.application-detail__card dd{margin:0;font-weight:650}.application-detail__context{display:grid;align-content:start;gap:.7rem}.application-detail__context h2{margin:0;font-size:1rem}.application-detail__context p{margin:0;line-height:1.5}.application-detail__status{padding:1rem 0;color:rgba(23,34,26,.65)}.application-detail__status--error{color:#8d2f25}.application-detail__status button{margin-left:.6rem;border:0;background:none;text-decoration:underline;cursor:pointer}@media(max-width:700px){.application-detail__header{align-items:start;flex-direction:column}.application-detail__grid{grid-template-columns:1fr}}
</style>
