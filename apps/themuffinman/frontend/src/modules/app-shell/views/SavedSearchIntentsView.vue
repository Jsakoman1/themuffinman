...
const remove = async (item: SavedSearchIntent) => { if (!await confirmAction("Delete this saved search?", "Delete saved search")) return; try { await userShellApi.deleteSavedSearchIntent(item.id); items.value = items.value.filter(current => current.id !== item.id); feedback.value = "Saved search deleted." } catch { error.value = "Could not delete this search." } }
onMounted(() => void load())
</script>
...
<style scoped>.saved-searches{display:grid;gap:1rem;max-width:48rem}.eyebrow{margin:0 0 .3rem;color:var(--text-muted);font-size:.76rem;font-weight:650;letter-spacing:.08em;text-transform:uppercase}h1{margin:0;font-size:clamp(1.55rem,2.5vw,2.3rem);letter-spacing:-.075em}.saved-searches header p:last-child{color:var(--text-muted)}.list{display:grid;gap:.45rem}.list article{display:flex;justify-content:space-between;gap:1rem;padding:.85rem;border:1px solid var(--border-subtle)}article div:first-child{display:grid;gap:.25rem}small{color:var(--text-muted)}button{border:1px solid var(--border-subtle);border-radius:999px;padding:.45rem .7rem;background:transparent;font:inherit;cursor:pointer}.actions{display:flex;gap:.35rem}.success{color:var(--success)}.error{color:var(--danger)}</style>
