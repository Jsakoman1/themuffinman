...
<template>
  <section class="chat-surface">
    ...
    <div class="chat-surface__layout">
      ...
      <form v-if="selectedId" class="chat-surface__composer" @submit.prevent="send"><p v-if="replyingTo" class="chat-surface__replying">Replying to {{ replyingTo.senderUsername }} <button type="button" @click="replyingTo = null">Cancel</button></p><input v-model="draft" placeholder="Write a message." maxlength="2000"><label class="chat-surface__attachment">{{ isUploadingAttachment ? "Uploading…" : attachment ? attachment.attachmentName : "Attach" }}<input type="file" accept="image/*,.pdf,.txt" @change="uploadAttachment" :disabled="isUploadingAttachment"></label><img v-if="attachmentPreviewUrl" class="chat-surface__attachment-preview" :src="attachmentPreviewUrl" alt="Selected attachment preview"><button type="button" v-if="attachment" @click="removeAttachment">Remove</button><button type="submit" :disabled="isUploadingAttachment">Send</button></form>
    </div>
  </section>
</template>

<style scoped>
.chat-surface{display:grid;gap:1rem}.chat-surface__header{display:flex;justify-content:space-between}.chat-surface__header-actions{display:flex;gap:.4rem}.chat-surface__eyebrow{margin:0 0 .3rem;color:rgba(23,34,26,.55);font-size:.76rem;font-weight:650;letter-spacing:.08em;text-transform:uppercase}h1{margin:0;font-size:clamp(1.55rem,2.5vw,2.3rem);letter-spacing:-.075em}.chat-surface__direct-create,.chat-surface__group-create{display:flex;gap:.45rem;flex-wrap:wrap;align-items:center;padding:.7rem;border:1px solid rgba(23,34,26,.08);border-radius:.8rem;background:rgba(255,255,255,.62)}.chat-surface__direct-create input,.chat-surface__group-create input{min-width:14rem;flex:1;border:1px solid rgba(23,34,26,.14);border-radius:999px;padding:.6rem .75rem;font:inherit}.chat-surface__direct-create button,.chat-surface__group-create button{border:1px solid rgba(23,34,26,.12);border-radius:999px;padding:.45rem .7rem;background:#17221a;color:#f8f8f4;font:inherit;font-size:.78rem;cursor:pointer}.chat-surface__layout{display:grid;grid-template-columns:minmax(14rem,20rem) minmax(0,1fr);gap:1rem;min-height:28rem}.chat-surface__index,.chat-surface__thread{border:1px solid rgba(23,34,26,.08);border-radius:1rem;background:rgba(255,255,255,.62);padding:.8rem}.chat-surface__index{display:grid;align-content:start;gap:.25rem}.chat-surface__conversation{display:grid;gap:.2rem;padding:.7rem;border-radius:.7rem}.chat-surface__conversation span,.chat-surface__message-author{color:rgba(23,34,26,.55);font-size:.78rem}.chat-surface__conversation--active{background:rgba(214,228,218,.7)}.chat-surface__thread{display:grid;align-content:start;gap:.55rem}.chat-surface__message{max-width:72%;padding:.65rem .8rem;border-radius:.8rem;background:rgba(23,34,26,.06)}.chat-surface__message--own{justify-self:end;background:#17221a;color:#f8f8f4}.chat-surface__message p{margin:.25rem 0 0}.chat-surface__message--own .chat-surface__message-author{color:rgba(248,248,244,.65)}.chat-surface__more,.chat-surface__older{justify-self:start;border:1px solid rgba(23,34,26,.12);border-radius:999px;background:transparent;padding:.5rem .75rem;cursor:pointer}.chat-surface__status{padding:1rem 0;color:rgba(23,34,26,.62)}.chat-surface__status--error{color:#8d2f25}.chat-surface__status button{margin-left:.6rem;border:0;background:none;color:inherit;text-decoration:underline;cursor:pointer}@media(max-width:700px){.chat-surface__layout{grid-template-columns:1fr}.chat-surface__index{max-height:14rem;overflow:auto}.chat-surface__message{max-width:88%}}
.chat-surface__message-actions{display:flex;gap:.3rem;margin-top:.45rem}.chat-surface__message-actions button,.chat-surface__composer button{border:1px solid rgba(23,34,26,.12);border-radius:999px;padding:.35rem .6rem;background:transparent;font:inherit;font-size:.75rem;cursor:pointer}.chat-surface__composer{display:flex;gap:.45rem;grid-column:2;align-items:center;flex-wrap:wrap}.chat-surface__composer input{flex:1;min-width:12rem;border:1px solid rgba(23,34,26,.14);border-radius:999px;padding:.65rem .8rem;font:inherit}.chat-surface__composer button{background:#17221a;color:#f8f8f4}.chat-surface__attachment{display:inline-flex;align-items:center;border:1px solid rgba(23,34,26,.12);border-radius:999px;padding:.35rem .6rem;font-size:.75rem;cursor:pointer}.chat-surface__attachment input{display:none}.chat-surface__attachment-preview{width:3.5rem;height:3.5rem;object-fit:cover;border-radius:.55rem;border:1px solid rgba(23,34,26,.14)}
</style>
<style scoped>
.chat-surface{max-width:78rem}.chat-surface__layout{min-height:clamp(32rem,68vh,48rem);grid-template-rows:minmax(0,1fr) auto}.chat-surface__index,.chat-surface__thread{box-shadow:0 14px 34px rgba(23,34,26,.05)}.chat-surface__index{background:rgba(248,250,245,.78)}.chat-surface__conversation{border:1px solid transparent;transition:background-color 140ms ease,border-color 140ms ease}.chat-surface__conversation:hover{background:rgba(255,255,255,.9);border-color:var(--border-subtle)}.chat-surface__conversation strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.chat-surface__thread{overflow:auto;align-content:end;align-items:start;background:linear-gradient(180deg,rgba(255,255,255,.72),rgba(236,241,233,.58))}.chat-surface__message{position:relative;border-radius:1rem 1rem 1rem .25rem;box-shadow:0 4px 14px rgba(23,34,26,.04)}.chat-surface__message--own{border-radius:1rem 1rem .25rem 1rem}.chat-surface__message-actions{opacity:.7;transition:opacity 140ms ease}.chat-surface__message:hover .chat-surface__message-actions,.chat-surface__message:focus-within .chat-surface__message-actions{opacity:1}.chat-surface__composer{position:sticky;bottom:0;z-index:2;padding:.65rem;border:1px solid var(--border-subtle);border-radius:1rem;background:rgba(255,255,255,.94);backdrop-filter:blur(12px);box-shadow:0 12px 30px rgba(23,34,26,.08)}.chat-surface__composer input{background:transparent}.chat-surface__composer button[type=submit]{padding-inline:1rem}.chat-surface__attachment{background:var(--surface-muted)}.chat-surface__reply{margin:.25rem 0 0;color:rgba(23,34,26,.58);font-size:.75rem}.chat-surface__attachment-link{display:inline-block;margin-top:.25rem;font-size:.78rem}.chat-surface__replying{flex-basis:100%;margin:0;color:rgba(23,34,26,.62);font-size:.78rem}.chat-surface__replying button{margin-left:.4rem}
.chat-surface__header{align-items:center}.chat-surface__create-toggle,.chat-surface__leave{border:1px solid rgba(23,34,26,.12);border-radius:999px;padding:.5rem .75rem;background:#17221a;color:#f8f8f4;font:inherit;cursor:pointer}.chat-surface__group-create{display:grid;grid-template-columns:1fr 1fr auto;gap:.45rem;align-items:center;padding:.8rem;border:1px solid rgba(23,34,26,.08);border-radius:1rem;background:rgba(255,255,255,.62)}.chat-surface__group-create input{border:1px solid rgba(23,34,26,.14);border-radius:.65rem;padding:.6rem;font:inherit}.chat-surface__group-create button{border:0;border-radius:999px;padding:.6rem .8rem;background:#17221a;color:#f8f8f4;font:inherit;cursor:pointer}.chat-surface__group-create button:disabled{opacity:.5;cursor:wait}.chat-surface__candidate{display:flex;gap:.35rem;align-items:center;font-size:.85rem}.chat-surface__candidate input{width:auto}.chat-surface__leave{justify-self:end;background:transparent;color:#8d2f25;font-size:.8rem}@media(max-width:700px){.chat-surface__group-create{grid-template-columns:1fr}.chat-surface__candidate{grid-column:1}}
