...
class PersonalShortcutServiceTest {
    @Test void pinUsesVisibilityScopedQuestReadBeforePersisting() {
        var repository = mock(PersonalWorkspaceShortcutRepository.class); var quests = mock(WorkmarketQuestReadService.class); var user = new AppUser(); user.setId(4L);
        ...
        new PersonalShortcutService(repository, quests).pinQuest(7L, user);
    ...
    @Test void omitsDeletedOrNoLongerVisiblePinnedQuestWithoutFailingTheWorkspaceRead() {
        ...
        var shortcuts = new PersonalShortcutService(repository, quests).getMine(user);
