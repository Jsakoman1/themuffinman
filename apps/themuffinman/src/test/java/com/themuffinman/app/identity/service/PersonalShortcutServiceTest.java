...
import com.themuffinman.app.identity.repository.AppearancePreferenceRepository;
...
class PersonalShortcutServiceTest {
    @Test void pinUsesVisibilityScopedQuestReadBeforePersisting() {
        var repository = mock(PersonalWorkspaceShortcutRepository.class); var railPreferences = mock(WorkspaceRailPreferenceRepository.class); var appearancePreferences = mock(AppearancePreferenceRepository.class); var quests = mock(WorkmarketQuestReadService.class); var user = new AppUser(); user.setId(4L);
        ...
        new PersonalShortcutService(repository, railPreferences, appearancePreferences, quests).pinQuest(7L, user);
    ...
    @Test void omitsDeletedOrNoLongerVisiblePinnedQuestWithoutFailingTheWorkspaceRead() {
        ...
        var shortcuts = new PersonalShortcutService(repository, railPreferences, appearancePreferences, quests).getMine(user);
    ...
    @Test void keepsRailPreferenceViewerScopedAndRejectsUnsafeWidths() {
        ...
        var service = new PersonalShortcutService(repository, railPreferences, appearancePreferences, quests);
