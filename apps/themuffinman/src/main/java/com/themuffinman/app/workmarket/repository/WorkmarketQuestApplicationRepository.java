...
public interface WorkmarketQuestApplicationRepository extends JpaRepository<QuestApplication, Long> {

    @Query("select qa from WorkmarketQuestApplication qa join fetch qa.quest q join fetch q.creator join fetch qa.applicant where qa.quest.id = :questId")
    ...
    @Query("select qa from WorkmarketQuestApplication qa join fetch qa.quest q join fetch q.creator join fetch qa.applicant where qa.quest.id = :questId and qa.applicant.id = :applicantId and qa.status = :status")
    ...
    @Query("select qa from WorkmarketQuestApplication qa join fetch qa.quest q join fetch q.creator join fetch qa.applicant where qa.id = :id and qa.quest.id = :questId")
    Optional<QuestApplication> findForQuestApplicationDetail(Long id, Long questId);

    @Query("select qa from WorkmarketQuestApplication qa join fetch qa.quest q join fetch q.creator join fetch qa.applicant where qa.id = :id")
    Optional<QuestApplication> findForApplicationDetail(Long id);

    @Query("select qa from WorkmarketQuestApplication qa join fetch qa.quest q join fetch q.creator join fetch qa.applicant where qa.quest.id = :questId and qa.status = :status")
    List<QuestApplication> findForQuestApplicationsByStatus(Long questId, QuestApplicationStatus status);

    @Query("select qa from WorkmarketQuestApplication qa join fetch qa.quest q join fetch q.creator join fetch qa.applicant order by qa.createdAt desc")
