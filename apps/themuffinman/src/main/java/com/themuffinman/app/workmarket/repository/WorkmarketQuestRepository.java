...
public interface WorkmarketQuestRepository extends JpaRepository<Quest, Long> {

    @Query("select distinct q from WorkmarketQuest q join fetch q.creator left join fetch q.images left join fetch q.visibleToCircles")
    List<Quest> findForQuestList();

    @Query("select distinct q from WorkmarketQuest q join fetch q.creator left join fetch q.images left join fetch q.visibleToCircles where q.id in :ids")
    List<Quest> findForQuestListByIds(@Param("ids") List<Long> ids);

    @Query("select distinct q from WorkmarketQuest q join fetch q.creator left join fetch q.images left join fetch q.visibleToCircles where q.id = :id")
    ...
    @Query("select distinct q from WorkmarketQuest q join fetch q.creator left join fetch q.images left join fetch q.visibleToCircles where q.creator.id = :creatorId and q.status = :status order by q.id desc")
