...
public class BusinessConfirmBookingUseCase {
    ...
    public com.themuffinman.app.business.dto.BusinessBookingResponseDTO execute(Long bookingId, AppUser currentUser) {
        ...
        if (!businessBookingPolicyService.loadOrCreatePolicyEntity(currentUser).isAllowOwnerManualApproval()) {
            throw ServiceErrors.conflict("Owner approval is disabled by booking policy");
        }
        if (booking.getStatus() != BusinessBookingStatus.PENDING_CONFIRMATION) {
            throw ServiceErrors.conflict("Only pending bookings can be confirmed");
        }
