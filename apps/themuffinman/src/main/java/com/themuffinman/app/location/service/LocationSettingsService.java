...
public class LocationSettingsService {
    ...
    private List<AppUser> resolveExactVisibleUsers(AppUser owner, List<Long> userIds) {
        ...
        for (AppUser candidate : users) {
            ...
            if (circleRelationshipReadService.getRelationWithUser(owner, candidate.getId()).getRelationStatus() != CircleRelationStatusDTO.CIRCLE) {
    ...
    private void applyLookupCandidateToQuest(com.themuffinman.app.workmarket.model.Quest quest, LocationLookupCandidateDTO candidate) {
        if (candidate == null) {
            return;
        }
