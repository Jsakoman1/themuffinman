...
templates = Dora::GeneratedFeatureManifest::TEMPLATE_IDS.to_h { |id| [id, Digest::SHA256.hexdigest(id)] }
...
first = Dora::GeneratedFeatureManifest.build!(feature: feature, migration_version: "1", template_digests: templates)
second = Dora::GeneratedFeatureManifest.build!(feature: feature, migration_version: "1", template_digests: templates)
abort "manifest is not deterministic" unless first == second
abort "migration path is missing" unless first.fetch("outputs").any? { |output| output.fetch("path") == "backend/src/main/resources/db/migration/V1__create_supply.sql" }
puts "Dora generated feature manifest test passed (confirmed input, explicit migration version, template hashes, exact outputs, and unresolved obligations are deterministic)."
