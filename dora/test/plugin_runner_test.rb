...
Dir.mktmpdir("dora-plugin-runner") do |root|
  ...
  manifest = {"kind" => "dora_plugin_manifest", "version" => 1, "plugins" => [{"id" => "alpha", "entrypoint" => "plugins/alpha.rb", "source_roots" => [{"id" => "source", "path" => "src"}], "inputs" => {"mode" => "test"}, "output" => {"kind" => "static-analysis-report"}}]}
  ...
  abort "declared plugin did not run" unless result.fetch("output").include?("alpha plugin ran")
