...
Dir.mktmpdir("dora-work-artifact-audit") do |sandbox|
  ...
  before = [valid, malformed, unsupported, incomplete].to_h { |path| [path, File.binread(path)] }

  report = Dora::WorkArtifactAudit.inspect!(project_root: sandbox, paths: ["docs/work"], schema_path: SCHEMA, observed_at: OBSERVED_AT)
