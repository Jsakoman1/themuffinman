...
Dir.mktmpdir("dora-agent-session-command") do |root|
  ...
  abort "agent-session command failed or mutated work" unless status.success? && session.dig("work", "selected", "id") == "note" && session.fetch("omitted").include?("work mutation")
end
