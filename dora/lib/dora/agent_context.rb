...
module Dora
  class AgentContext
    def self.build!(project_root:, work_plan:, task_id:)
      ...
      {"kind" => "dora_agent_context", "version" => 1, "task" => task.slice("id", "title", "observable_outcome", "dependencies", "required_paths", "validation", "evidence_boundary"), "knowledge" => {"product" => knowledge.fetch("product_brief").slice("product", "user_problem", "intended_outcomes", "non_goals", "unanswered_decisions"), "domain" => knowledge.fetch("domain_library").slice("vocabulary", "invariants", "permission_rules", "workflows"), "agent" => knowledge.fetch("agent_profile").slice("authority_limits", "evidence_requirements", "implementation_order")}, "citations" => citations, "omitted" => ["undeclared repository files", "product inference", "completion conclusion"]}.freeze
