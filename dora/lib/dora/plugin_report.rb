...
module Dora
  class PluginReport
    ...
    def self.build!(plugin_id:, inputs:, findings:, output:, finding_context: {})
      ...
      {"kind" => "dora_plugin_report", "version" => 1, "plugin" => plugin_id, "inputs" => inputs, "findings" => findings.each_with_index.map { |finding, index| standard_finding(plugin_id, finding, index, finding_context) }, "output" => output, "finding_contract" => "dora_finding", "completion_boundary" => COMPLETION_BOUNDARY}
    end
