...
- Owners configure resource pools, individual resources, and offering resource requirements directly from service setup. The owner API supports typed read, create, update, and delete operations for all three resource configuration families; every mutation rechecks business ownership and returns the refreshed configuration. A reschedule rechecks and rebuilds the live resource assignment for the new interval atomically; the original booking snapshot remains unchanged as historical pricing and demand evidence.
- Deleting a pool follows the database contract: linked resources become unpooled and linked offering requirements are removed. A resource retained by an existing booking cannot be deleted; the owner must deactivate it instead so booking history stays intact.
- The owner Resources step now shows pools, individual resources, and only the selected service's requirements together. Owners can create and edit all three, activate or deactivate pools and resources, and use confirmed destructive actions. New requirements use automatic assignment because the booking runtime currently selects any active matching resource; the Web surface does not promise fixed-resource selection.
...
Home shows at most one actionable recovery handoff. It states the resumable item's prepared title, its next-step summary, and the action that opens it; when no work needs resuming, it offers the concrete low-pressure next step `Find help` rather than a passive placeholder. Its schedule follow-up shows only the next calendar item and links to the dedicated Calendar route; detailed notification lists and calendar grids remain in their owned surfaces.

The unified Calendar presents events in the viewer's current local time by default. It labels an overridden calendar timezone explicitly, while event aggregation, source permissions, and business booking timezones remain backend-owned.
...
SideJobs are the human-facing presentation of the existing Work quest workflow. A person uses `Find help` to browse requests, `My posts` to manage requests they published, and `My help offers` to track offers they made. They can post a private draft only after review and offer to help without creating a second persistence model. The backend remains authoritative for eligibility, visibility, allowed actions, assignment, pricing validation, and lifecycle transitions. Fresh desktop and mobile browser evidence is retained in `docs/runtime-evidence/sidejobs-human-first-*-runtime.json`.
...
Module navigation closeout: the authenticated workspace rail is a stable list of user intentions. It names the work area `SideJobs`, the owner area `Offer services`, and the customer area `Find services`; their canonical routes and permissions are unchanged. Things, People, and Rides present their subroutes as local tabs inside module content, so a person changes their main intent globally and their task locally without losing authorized actions.
