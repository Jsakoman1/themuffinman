...
- `users` files=36 hits=344: Extend existing `TestFixtures` coverage for repeated users setup instead of adding another local helper.
- `quests` files=14 hits=123: Extend existing `TestFixtures` coverage for repeated quests setup instead of adding another local helper.
...
- `location-settings` files=9 hits=36: Extend existing `TestFixtures` coverage for repeated location-settings setup instead of adding another local helper.
...
- `apps/themuffinman/src/test/java/com/themuffinman/app/social/service/CircleServiceTest.java` score=155 focus=users,circles
