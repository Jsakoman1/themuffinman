...
- Feature completion manifests embed structured closeout evidence under `validationEvidence`, `generatedArtifacts`, `planCompletion`, and `closeoutDecision`; `make feature-closeout-audit manifest=<manifest-file>` first runs the matching plan completion audit and then hard-fails completed manifests that lack required command evidence, skipped-check reasons, profile-specific validation, completed plan evidence, or clean artifact bucket ownership.
- Version 2 plan closeout requires `machine_baseline_ref` to match the completion-evidence baseline and verifies one shared runtime or tooling path across declared plan code paths, manifest `artifacts.codePaths`, and the Git diff after that baseline. Master plans recursively require every explicit child plan to pass before they can close.
- Implementation batch ledgers retain append-only history but emit a unique run identifier and a run-scoped JSON ledger for each invocation. Master child discovery reads only the explicit `Plan inventory` and `Plan Breakdown` sections, so historical references elsewhere do not become hidden closeout dependencies.
...
- Validation and closeout evidence is independently auditable with `make audit-validation-evidence-quality`, which fails missing completion evidence, non-final completion status, unresolved task checkboxes, and placeholder evidence.
- The generated validation evidence summary should carry any notable child issue rollups directly instead of collapsing them into one `incomplete` label.
- Closeout summary generation lives in the validation and evidence reports and should write a short Markdown summary from manifest artifacts, validation evidence, docs delta, generated-artifact evidence, backlog delta, residual risks, and the current changed-file list.
...
- Codebase capsule generation is available through `make codebase-capsule`, producing a short read-first Markdown and JSON summary with repository layout, active conventions, open backlog/batch context, and preferred first commands.
...
- Batch/code maps are available through the compact context and generated audit summaries, which map one implementation slice to likely code files, living docs, generated artifacts, validations, manifests, and related audit targets.
...
- Broad, long-running, high-complexity, multi-layer, or high-risk changes use operating-model implementation checkpoints for the initial batch, first backend slice, first frontend slice, docs/artifacts sync, validation, and commit boundary.
...
- `make control-start` generates `plan-index` and `control-start` snapshots from active `.agents/*-plan.md` files, the current audit summary, and temporary work-product metadata; `make codex-context` consumes that snapshot for plan and tooling requests without recursively invoking itself.
