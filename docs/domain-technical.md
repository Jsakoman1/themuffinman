...
- Entity surface parity is audited against request DTOs and authenticated controllers, not only database models. Business profile/offerings/policy/availability, Rides circle visibility, Things availability, and Work application review fields are now represented in Web forms; untyped Business resource configuration endpoints remain an explicit follow-up boundary until their request schema is typed.
