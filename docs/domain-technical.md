...
- Home composes a viewer-scoped active summary from existing Work, Business, Services, People, Things, and Rides read endpoints. `HomeHubView` renders six compact module cards, including backend-provided command-catalog actions and concise empty states; it routes each row to an existing canonical module route and does not duplicate module discovery, navigation, favorites, or recent-context history.
- `workspace-navigation-v1` now publishes the personal module tree in the order Home, Work, Business, Services, Things, People, and Rides. `WorkspaceNavigationService` emits individual owned business profiles under Business and individual favorite businesses under Services through viewer-scoped services. Things, People, and Rides child routes are emitted by the same backend navigation DTO; Things and Rides open on their `My` route by default, while the frontend does not synthesize missing children. Home composes six category cards from existing reads and the backend command catalog, while `HomeCalendarPreview` owns a compact `/calendar` projection with Week as the default mode and Day/Month alternatives.
...
- `BusinessFavorite` is keyed by owner and business profile and is idempotent. Favorite reads expose only public business summary fields, and `GET /business/favorites/me/directory` provides the viewer-scoped, queryable active-profile projection used by favorite discovery.
