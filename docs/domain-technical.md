...
- `workspace-navigation-v1` publishes the personal module tree in the order Home, Work, Business, Services, Things, People, and Rides. `WorkspaceNavigationService` emits individual owned business profiles under Business and individual favorite businesses under Services through viewer-scoped services. Things, People, and Rides child routes are emitted by the same backend navigation DTO; Things and Rides open on their `My` route by default, while the frontend does not synthesize missing children. Desktop consumes this tree through the workspace rail; mobile exposes a fixed primary-tab subset and recovers the complete tree in an accessible drawer.
