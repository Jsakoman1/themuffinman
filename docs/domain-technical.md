...
## Design and client authority

The primary Web design/interaction contract is
`docs/apple-desktop-design-reference.md`. It does not move domain authority into
the frontend: backend APIs remain authoritative for permissions, read models,
commands, search scopes, workflow transitions, Calendar aggregation, booking,
pricing, Vision actions, and recovery metadata. The reference is a presentation
and client-parity constraint for every redesigned surface.
...
- `calendar`: the authenticated `/calendar` projection aggregates viewer-visible business bookings, scheduled visible quests, and circle-visible rides with optional source and business filters; business rules remain in source services. The projection supplies source descriptors (`key`, human label, color) and ISO offset-local start/end values for every event, so clients do not infer source names or calendar-local placement.
...
- `WorkmarketWorkerManagementService` remains authoritative for owner/admin permission, eligible quest states, approved-worker lookup, and replacement transitions. Its stale conflicts use typed API codes (`QUEST_WORKER_STATE_STALE`, `WORKER_ASSIGNMENT_STALE`, `REPLACEMENT_APPLICATION_STALE`) through `CodedResponseStatusException`; allowed actions and the pending replacement collection remain backend-prepared on application DTOs.
- Owner lifecycle transitions use explicit backend commands (`PATCH /quests/{id}/assign` and `PATCH /quests/{id}/reopen`); Web must not submit lifecycle status values through the general quest-edit request.
- People search and circle search must render connection controls from `ProfilePrimaryActionDTO.enabled`; relation labels are presentation text and must not be used as permission or eligibility gates.
- `QuestApplicationDraftRulesViewDTO` is the source of truth for the quest-detail application form's required fields, minimum proposed price, and suggested price. `WorkQuestDetailView` consumes those flags and does not derive them from `awardAmount`.
- `ActionResultDTO.message` is the source of truth for successful quest-action feedback; the Web client does not duplicate action-to-message mappings.
- `Quest.version` is the optimistic-concurrency token for owner/admin `PUT /quests/{id}` updates. `WorkmarketUpdateQuestUseCase` requires the DTO `resourceVersion`, rejects mismatches with `STALE_RESOURCE`/HTTP 409 before applying field changes, and `QuestResponseDTO` returns the current token for the next write. Vision's dedicated title mutation remains a separate service path and is not covered by this DTO guard.
- `WorkQuestCreateView` and `WorkQuestDetailView` map the scheduling, terms, audience, location, and assignment portions of the `QuestRequestDTO` editing surface. Create additionally exposes backend-authorized circle selection and image data URLs; local datetime inputs are converted to ISO instants at the mutation boundary. Quest image replacement and some administrative-only request fields remain outside the regular user edit surface; backend validation remains authoritative and its response message is shown to the user.
- Business profile latitude and longitude are nullable contract fields. Creating a profile may omit coordinates; discovery and map-adjacent UI must preserve the absence rather than inventing a location.
- Entity surface parity is audited against request DTOs and authenticated controllers, not only database models. Business profile/offerings/policy/availability/resources, Rides circle visibility, Things availability, and Work application review fields use typed client and backend contracts; Web resource editing remains a separate presentation slice.
- Route parity is checked against `router.ts` and module navigation metadata: a route is not considered complete merely because it exists; it needs a discoverable authenticated entry, a form/detail handoff, and backend-owned error/action recovery.
- Workmarket application-news events carry an event UUID and persist a unique delivery key, so repeated delivery does not insert a second news row. This is consumer idempotency only; after-commit publication remains process-local and crash/replay durability is still pending.
...
- Route identity is part of every frontend read boundary. Components reused by Vue Router must watch dynamic params and query context, reload the backend projection for the new identity, and invalidate stale selection or mutation state; direct URL smoke tests are insufficient for this invariant.
...
- The authenticated shell fails closed when `workspace-navigation-v1` is unavailable or empty. There is no frontend fallback module tree or local child-route list; the shell renders an explicit backend-unavailable state instead.
...
- Workmarket notification-derived Activity rows use the same backend destination metadata as Notifications. `APPLICATION` and available `QUEST` targets receive canonical detail routes; unavailable targets fall back to `/notifications`, preventing stale Activity links from bypassing target-existence and authorization boundaries.
...
- `RuntimeObservabilityFilter` is an opt-in dev measurement boundary. When `app.runtime-observability.enabled=true`, it exposes per-request `X-Runtime-Query-Count` and `X-Runtime-Request-Duration-Ms` headers using Hibernate statistics and buffered responses; production defaults keep it disabled, and the headers must not be interpreted as scale or provider-timing evidence.
...
- Vision turn requests may carry workspace context, source, and return path as bounded Web presentation context. `VisionConversationService` sanitizes the source and local return path into the versioned handoff contract; current route context may inform OpenAI semantic interpretation through the backend-owned orchestration request, but it never grants authority, bypasses permission checks, or replaces deterministic target resolution.
...
- `/notifications` now consumes the backend `/news/me` contract, preserving unread state and mark-read actions while following backend-owned destination metadata. Workmarket news read mapping checks referenced quest/application existence and downgrades unavailable or deleted targets to `QUEST_LIST`; the frontend does not derive a quest route from a stale `questId`.
...
- `HomeHubView` consumes the single viewer-scoped `workspace-home-v1` launcher contract rather than aggregating module dashboards in the browser. It renders only permitted purpose cards, one compact attention recovery handoff, and a next-calendar-item link; module discovery, complete activity, and full calendar rendering remain owned by their canonical routes.
- `workspace-navigation-v1` publishes the personal module tree in the order Home, SideJobs, Offer services, Find services, Things, People, and Rides. These are human-facing labels for the existing `work`, `business`, and `services` module ids and routes; they do not alter permissions or ownership. `WorkspaceNavigationService` emits individual owned business profiles under Offer services and individual favorite businesses under Find services through viewer-scoped services. Things, People, and Rides child routes are emitted by the same backend navigation DTO; Things and Rides open on their `My` route by default, while the frontend does not synthesize missing children. Desktop consumes this tree through the workspace rail; mobile exposes a fixed primary-tab subset and recovers the complete tree in an accessible drawer.
- `WorkspaceHomeReadService` owns `workspace-home-v1` at `GET /workspace/home`. It filters the four launcher route definitions against the viewer's existing backend command catalog and returns only the greeting plus permitted launcher metadata; it performs no mutation and does not derive an action from browser state.
- The Calendar client reads the backend projection timezone for date grouping and event formatting, then labels it as `Your local time` only when it matches the browser's resolved timezone. A different projection timezone is shown explicitly; the client does not alter event instants or booking timezone rules.
- `AuthenticatedShellView` keeps route context in the active rail and child surface; it no longer renders a separate Back/current-location topbar. `AccountMenu` is mounted as the shared navbar Account section on desktop and mobile, so identity, settings, activity, notification settings, setup, and logout have one account-owned presentation path.
- `ActivityReadService.getRecent` is Home’s bounded, viewer-scoped resume feed. It is derived from already permitted activity/news and non-dismissed Vision conversations; the browser must not synthesize or persist a separate recent-object history.
...
- OpenAI is the only production semantic interpreter for `/vision` and VisionForWeb prompt understanding across quests, circles, applications, profiles, chat, and Web navigation. A deterministic local parser may run only behind an explicit development/test profile for labeled synthetic fixtures and contract tests; production fails closed when OpenAI is unavailable instead of translating, classifying, or routing locally. The current contract and runtime boundary are authoritative in the Vision and runtime registries.
...
- `VisionPromptUnderstandingResult` and `VisionConversationTurnResponseDTO` expose provider/status information for OpenAI success, provider-unavailable recovery, and contract rejection; production responses must not claim local semantic rescue or local emergency routing.
...
- Development-only deterministic parser fixtures may be English-only and synthetic; arbitrary-language normalization and all production user input are handled by OpenAI. If OpenAI is unavailable, Vision pauses and preserves retryable conversation state.
...
- `BusinessProfileService` owns profile validation, slug normalization, active-directory visibility, booking-enabled flags, timezone/contact fields, and rich-text description sanitization. Authenticated customer-facing Services directory reads exclude profiles owned by the viewer; owner workspaces remain the sole management surface for those profiles.
...
- Ride offers and Thing listings or borrow requests also project those existing action vocabularies as `ClientActionDTO` lists. The backend supplies labels, confirmation copy, tones, and outcome guidance; the existing action enums and services remain the authorization and transition authority.
...
- Shared `ClientActionDTO` and `ClientActionToneDTO` provide a device-neutral action contract with labels, confirmation requirements, outcome guidance, tone, and disabled reasons; domain-specific action enums remain available for compatibility. Business booking and Work application response DTOs populate those descriptors from their backend-owned allowed actions, so clients render the next-step explanation without reconstructing workflow meaning.
- The human-first Web static contract validates direct object opening, visible personal navigation, specialized Chat and Calendar routes, backend-prepared actions, and explicit surface-renderer archetypes before runtime evidence is recorded.
...
- `BusinessFavorite` is keyed by owner and business profile and is idempotent. A viewer cannot favorite their own profile, and favorite discovery excludes any legacy self-favorite rows; reads expose only public business summary fields, while `GET /business/favorites/me/directory` provides the viewer-scoped, queryable active-profile projection used by customer-facing favorite discovery.
...
- `OnboardingProgressService` persists only the allow-listed setup step plus skip/completed flags. `ActivityReadService` projects existing viewer-owned news and Vision conversations and stores only bounded resume-key dismissals; `AttentionCenterService` maps the same viewer-scoped news stream used for its unread count.
...
- Visible Web actions are governed by the retained `docs/work/action-contract-matrix.yaml`, backend action descriptors, and the frontend route registry.
...
- Home metrics and their collection destinations use the retained `docs/work/home-query-scope-contract.yaml`; a frontend label cannot create or infer a scope.
...
- `BusinessResourceController` and `BusinessResourceService` own the typed resource configuration boundary. Pool, resource, and offering-requirement mutations are profile-owner scoped and return `BusinessResourceConfigurationDTO`; requests use dedicated DTOs rather than raw maps.
- Pool deletion preserves the V84 foreign-key behavior: resources are detached and requirements targeting that pool are deleted. Resource deletion remains restricted by V88 booking assignments and returns a conflict instead of discarding retained booking history.
...
`GET /attention/me` remains the backend-owned compatibility projection for unread count and viewer-scoped news items. `AttentionCenterService` uses `WorkmarketQuestNewsService` plus `WorkmarketQuestNewsMgr`, so count and items share one recipient-scoped source. The Web Updates surface renders these items directly; resumable activity is served separately by `ActivityReadService` for Home.
...
### SideJobs presentation boundary

`Quest`, `QuestApplication`, their controllers, and their service-layer lifecycle remain the canonical Work domain. The Web labels these authorized read models as SideJobs and help requests. `QuestPresentationDTO` supplies safe, viewer-specific action guidance and attention labels; the frontend must not derive permission, status transitions, or request outcomes. The SideJobs runtime script exercises only authenticated presentation routes and records desktop/mobile overflow and browser-error boundaries separately from backend workflow tests.
...
`BusinessServiceSchemaView` loads the typed `BusinessResourceConfigurationDTO` for its selected owned profile and delegates resource mutations to `BusinessResourcePoolEditor`. Every successful mutation replaces the local configuration with the complete response rather than deriving state client-side. Requirement presentation is filtered to the selected offering, while pool and resource lists remain profile-wide. The owner can edit and toggle active state, and destructive operations use `AppActionDialog`; the backend remains authoritative for ownership, foreign-key effects, and retained-booking conflicts. New Web requirements submit `ANY_AVAILABLE`. Existing non-default assignment values are preserved on edit but are not presented as a supported fixed-resource workflow because `BusinessResourceAssignmentService` currently allocates the first available matching resources regardless of that stored label.
...
Responsive portability checkpoint: 2026-07-24. Calendar, Work, and the persistent Vision composer declare intentional desktop/mobile presentation modes; those modes are presentation metadata only, while read scope, permissions, and mutations remain API-owned.
System-behavior contract checkpoint: 2026-07-24. Reversible actions, draft recovery, capability metadata, consent, notifications, and client portability remain backend/API concerns.
Trust and recovery checkpoint: 2026-07-24. Backend-owned permissions and state transitions remain authoritative; clients expose concise scope metadata and retryable failure states.
The profile settings surface may progressively disclose independent preference groups in the frontend, but save boundaries, validation, privacy visibility, and location authorization remain owned by backend contracts.
...
### App-wide friendly presentation boundary

`FriendlyCollectionHeader`, `FriendlySummaryCard`, `DetailSurface`, `DetailSurfaceHeader`, and `TaskSurface` provide presentation-only orientation, hierarchy, and responsive spacing. They accept no permission, state-transition, availability, or pricing authority. Responsive composition may hide a competing pane on a narrow viewport, but canonical route reads and mutations still use the same backend DTOs and endpoint contracts as desktop.

`WorkspaceNavigationService` supplies only stable module entries for the authenticated rail. Module subroute links are presentation-owned local `ModuleTabs` definitions; their routes remain canonical backend-authorized navigation targets and do not derive permissions from the client.
...
`BusinessOfferingSetupService` exposes owner-authorized offering setup metadata at `GET /business/offerings/setup/me?businessProfileId=`. The read-only DTO carries request defaults, allowed presentation options, guided setup steps, field constraints, and readiness copy; it does not alter offering validation, quoting, availability, or booking transitions.

### Public Services calendar and review contract

`BusinessAvailabilityReadService.getPublicAvailabilityCalendar` exposes a bounded 1–31 day business-local projection at `GET /business/public/{slug}/availability/calendar`. The required `offeringId`, `fromDate`, `days`, and `view` (`MONTH`, `WEEK`, or `DAY`) inputs select presentation range only; they never select an owner-calendar projection. Each day exposes only local date, availability state, slot count, and safe derived slot windows. `UNAVAILABLE` days do not contain bookable slots. Existing booking records, customer identity/contact/note, exception reasons, resources, and capacity-consumption details are absent from this DTO and from the public controller path. The browser keeps the calendar cursor separately from the selected booking date; the existing preview/create services revalidate the selected slot. `business_review` has one unique `(business_booking_id, reviewer_user_id)` record; `BusinessReviewService` permits its write only to the booking customer after `COMPLETED`. Public review reads include reviewer username, stars, plain bounded comment, historical service title, and timestamps, never booking notes or customer contact data.
