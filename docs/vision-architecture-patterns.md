...
Do not let production fallback heuristics treat every prompt as description, reward, or location text just because the user spoke in one sentence. Deterministic fallback fixtures may exist only in an explicit development/test profile.
...
Shared prompt semantics should be reused across Vision and Admin Playground when the same normalization and intent-classification rules apply, so both surfaces consume one backend prompt-understanding boundary instead of maintaining separate production heuristics. Any deterministic local parser is development/test-only and must never be selected for production Vision input.
