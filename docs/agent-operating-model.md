...
- `make implementation-batch topic=<topic>` is the deterministic preparation wrapper for discovery, docs-sync preflight, manifest routing, validation preset selection, and recommendations. It performs closeout only when `closeout=true` is explicit and both plan and manifest paths are supplied.
- Version 2 implementation plans cannot close until `make audit-plan-completion plan=<plan-file> [manifest=<manifest-file>]` verifies the matching Git baseline and a shared declared code path across plan evidence, manifest `codePaths`, and the Git diff; a master plan also requires every explicit child plan to pass.
- Run `make implementation-batch`, `make closeout-preflight`, `make audit-plan-completion`, and `make feature-closeout-audit` from the repository root. The application Makefile owns only application-local targets.
